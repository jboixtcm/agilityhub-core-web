...
- **Decisió Jordi:** (06-09) **Proxy same-site + cookies httpOnly** — criteri qualitat: el refresh token no ha de ser accessible des de JS (amb WebCrypto un XSS encara pot *usar* la clau). Com que A11 posa Caddy davant de cada host de club, la ruta `/api/*` → core és configuració genèrica, no per club. Es retira l'emmagatzematge WebCrypto amb una tasca de correcció; E18 (pont OIDC) es manté. **Aplicat 09-09**: S01 v0.4 (R-01-06, §6 `tokenDelivery = COOKIE`), tasques api E1-T13 i web E1-W07.
...
- **Decisió Jordi:** (06-09) Admin inicial: **`jboix@agilitydoghub.com`** (només aquest). Requisit afegit: **s'ha de poder afegir/treure administradors de plataforma fàcilment via API** (`POST /platform/accounts` + gestió del rol `AGILITYHUB_ADMIN`). La resta de la recomanació, OK (cap admin automàtic dels rols de Learn, `anonymous@…` no s'importa, `locale = es`). **Aplicat 09-09**: `GET/PUT /platform/accounts/{id}/platform-roles` + CLI `identity:grant-platform-admin` avançats a E1 (api E1-T13); audit `PLATFORM_ROLES_CHANGED` a S14.
...
- **Decisió Jordi:** (06-09) **No es fa — no mesclem temes.** Els nivells són catàleg **100 % local i configurable de cada club** (el SaaS permet crear-los i gestionar-los, D11); **cap** `Level.agilityhubLevel` ni mapatge amb l'escala de Learn. `EASY·MEDIUM·HARD` i `GRAND·GARDEN` queden **només al domini de cursos/Learn** com a metadades del recorregut. Els rings dels clubs no tenen a veure amb els dels challenges del Hub; l'únic punt de contacte possible (facilitar el muntatge d'una pista d'un challenge) queda per a R2+ si mai interessa. **Supersedeix** l'`agilityhubLevel` del 03-09 (part D). **Aplicat 09-09**: S05 (§3, seed, D11, i18n), S19 (R-19-05/08), `MODEL_DADES_PLATAFORMA.md`; tasques api E2-T03 pas 0, web E2-W08.
...
### A28 · Data de cobrament de la remesa: el Josep diu **dia 1 del mes facturat** (supera la teva decisió del 05-09) — **tancada 09-09**
...
- **Decisió Jordi (09-09):** ✅ **les dues opcions s'han de suportar i han de ser fàcils de configurar** — «cada club és un món». `billing.sepa.collectionDayOfMonth` es queda com a paràmetre de club **editable des de D11** (bloc «Quotes, packs i remesa»), amb les dues semàntiques implementades i provades: `1–28` = aquell dia del mes facturat · `0` = últim dia (T-12-29b). **Cànic: `1`**, el que diu el Josep. En canviar el valor cal repassar `signup.text.paymentDay` (text de la pantalla 19, `localizedText` per club).

### A29 · Efectiu: el Josep diu **mai mensual, sempre per semestres naturals** (supera la teva decisió del 05-09) — **tancada 09-09**
...
- **Decisió Jordi (09-09):** ✅ **les dues opcions s'han de suportar i han de ser fàcils de configurar** — «cada club és un món». `billing.cashInvoicing` es queda com a paràmetre de club **editable des de D11**, amb `MONTHLY` i `SEMESTER` implementats i provats (T-12-03 cobreix els dos). **Cànic: `SEMESTER`**, el que diu el Josep, i el text de la pantalla 19 es manté. En canviar el valor cal repassar `signup.text.cashConditions` (`localizedText` per club).

### A30 · Vinculació amb Learn: preparada però no activada a R1 — **tancada 09-09**
- **Decisió Jordi (09-09):** el programari **ha d'estar preparat** per a la integració amb Learn des del primer dia (S19 sencera: client confidencial OIDC `learn`, `externalIds.learnUserId`, `agilityhub_account_id`, CLI `identity:import-learn`, ADR-010), però **no es vincula amb Learn ara**. No es carrega la base de dades de producció de Learn ni s'activa cap sincronització: es farà quan es passi d'un sol club (el Cànic) al SAAS multi-club.
- **Conseqüències**: **E1-T08** (adaptador al repo Laravel `agilityhub-api`) queda `not_open` sense data i **la porta E1 ja no l'espera** · `AGILITYHUB_ID_SYNC` es manté apagat i el client `learn` es registra sense callbacks de producció · la importació d'usuaris de Learn (E1-T07, dry-run ✅) es queda al codi i als tests però no s'executa mai contra dades reals · la fase 2 («Continua amb AgilityHub») i la fase 3 (retirar el login clàssic de Learn, S19 §13-5) es replantegen quan arribi el moment · no cal clonar `agilityhub-api` ni afegir cap bucle `learn` al kit.
- **Què NO canvia**: cap spec ni cap model. S19, ADR-010 i les regles R-19-* es queden tal com són. És una decisió de **calendari**, no d'abast: el dia que s'activi, no hi ha d'haver res a redissenyar.

### A31 · Proves en local amb Docker fins al release — **tancada 24-09**
- **Decisió Jordi (24-09):** «de moment estem treballant en docker i farem totes les proves en local, quan estigui acabat ja farem el release».
...
## Part B — Respostes del Josep (rebudes el 08-09-2026) ✅ les 21 del correu, tancades · **B29–B33 tancades el 24-09** (el Josep accepta les cinc propostes i demana el llistat del cas 5)
...
**✅ Resposta del Josep (24-09): «tira endavant amb la solució proposada en cada cas» i demana el llistat dels afectats pel cas 5.** Les cinc queden tancades amb l'assumpció aplicada (B31 passa de «sense modalitat» a `ABONAT_FAMILIAR`). Llistat del cas 5 lliurat **fora del Dropbox** (`~/Downloads/Canic-cas5-correus-compartits-2026-09-24.xlsx`): de les 20 adreces repetides, **17 semblen la mateixa persona amb dos gossos** (Playoff fa una fitxa per gos i la segona porta un altre NIF, en 11 casos començat per 0) i **3 són famílies** → decisió **E32**. El club hi confirma cada fila.
...
| E21 | Desplegament (nou a la llista de Jordi): `OIDC_MASTER_KEY` per entorn i les URL de callback dels clients OIDC (`core.oidc.clients[]`), `SENDGRID_API_KEY`, domini verificat · **+ `BOOKING_CALENDAR_KEY`** (24-09, E5-T02: signa els `.ics` de les reserves; obligatòria a staging/prod, l'app no arrenca sense) · `SIGNUP_CAPABILITY_KEY` | `.env.example` de l'api · `docs/DEPLOY.md` | — |

**Decisions de contracte del 24-09 (verificació d'E4-T04…E5-T05, primera nit amb l'executora Claude)** — revisa-les amb calma; marca `✗` la que vulguis canviar:

| # | Decisió | On | Per què |
|---|---|---|---|
| E22 | Treure «Reservable per entrenaments» o desactivar una pista amb reserves d'entrenament vives segueix **S09 R-09-13**: `422 RING_HAS_BOOKINGS` (corregit el 24-09: mana la regla 0 del catàleg d'errors) llevat que l'ADMIN enviï `cancelBookings: true` (les reserves passen a `CANCELLED_BY_CLUB / RING_NOT_RESERVABLE`). `RING_IN_USE` queda per a la resta de R-05-07 | S05 R-05-08 · T-05-28 · codi a E5-T09 | S05 i S09 es contradeien; la regla de S09 és posterior i deixa l'ADMIN resoldre-ho en un sol pas en comptes de bloquejar-lo |
| E23 | Estats HTTP explícits: `OVERRIDE_NOT_ALLOWED` 403, `JOB_UNKNOWN` 404, `SLOT_NOT_ON_GRID` 400 (ara surten 422 per la regla 0) | §1 de `CATALEG_ERRORS.md` + `ErrorCode`, a E5-T09 (els dos canvis junts perquè el test de contracte no es trenqui) | les taules d'endpoints d'S09/S15 ja deien aquests estats |
| E24 | Variables noves al catàleg de notificacions: N-15 `mode`, `entityId` · N-17 `class_description`, `ring_name` · N-36 `change` · N-47 `admin_text`, `change`; audiència de N-47 = «by = ADMIN» | `CATALEG_NOTIFICACIONS.md` (aplicat) | l'executora les necessitava per als textos (select de creat/anul·lat, FIFO/tots alhora) |
| E25 | Noms: `ClassBelowMinimum{classId}` (el catàleg d'esdeveniments mana sobre S15) · N-42 sense `date` · la purga d'exports deixa una làpida `EXPIRED` amb `purgeAt` immediat · `WaitlistEntry.cancelReason` + `MEMBER_LEFT` | S15, S08 §3 (aplicat) | alinear specs amb el catàleg i el model |
| E26 | Concurrència: R1 és **una sola instància de l'API** (ADR-003). Els carrils de bloqueig en memòria són acceptables per limitar la contenció, però els mecanismes de Mongo que diuen les specs (`$inc` + reintents, índex únic) s'han de provar amb els carrils apagats, i els carrils van per agregat, no per club. Si mai hi ha més d'una instància: reintents d'S07/S08 a 5 amb retrocés exponencial amb jitter (50 · 100 · 200 · 400 ms), la proposta que va mesurar E5-T07 (sense carrils, un 30–45 % d'una ràfega de 20 peticions acaba en `STALE_VERSION`) | E5-T07 | les revisions van veure que els tests de concurrència no toquen mai Mongo; amb més d'una instància no protegirien res |
| E27 | Revisió independent de cada tasca per un altre agent (per defecte l'altre model; si Codex no té crèdit, una sessió nova de Claude). Les troballes menors s'agrupen en tasques de seguiment (E5-T08, E5-T09) en comptes de fer rondes extra | `roadmap/reviews/`, `mac/review.sh` | més qualitat sense frenar la cua |

**Decisions del 24-09 a la tarda (verificació d'E5-T01…T08, E3-W03 i E4-W01)** — mateix criteri; marca `✗` la que vulguis canviar:

| # | Decisió | On | Per què |
|---|---|---|---|
| E28 | Prova de càrrega de l'obertura de diumenge (k6, porta E5): **300 abonats diferents** (ficticis) arriben durant els primers **5 s**, sobre 10 classes buides de 5 places; objectius p95 < 500 ms per bloquejar la plaça i < 800 ms per al flux sencer, i zero sobre-reserves. La ràfega «tots al mateix mil·lisegon» es manté com a prova d'estrès de correcció, amb latències informades però no bloquejants, i es tornarà a mesurar al servidor definitiu abans del release (A31) | `bin/e5-perf`, E5-T06 ronda 2 | al portàtil, 300 peticions exactament simultànies saturen el Mongo d'un sol node; 5 s és una arribada realista i la correcció ja es prova a part |
| E29 | Nivells fora de la progressió: camp nou `Level.progression` (per defecte sí; Teràpia = no), configurable a la targeta Nivells de D11. La descripció automàtica «D i sup.» (S06 R-06-03) només compta els nivells de la progressió; un conjunt amb un nivell de fora s'uneix amb «+» («G+Teràpia»), i els nivells de fora no surten a la taula de cobertura de D3 (R-06-06). El nivell «Pendent» de B32 també és de fora | S05 §3 i seed, S06 R-06-03 i T-06-02 (aplicat) · codi: E5-T11 (api), E4-W01 i E4-W06 (web) | amb Teràpia com a últim nivell, la regla literal no donava mai «D i sup.», que és el que ensenyen els mockups; així cada club decideix quins nivells són especials |
| E30 | Si el proveïdor de pagament falla en obrir el pagament d'una classe individual (`PAY_TO_BOOK`), la reserva s'anul·la i la plaça s'allibera a l'instant, però **no s'envia N-40**: l'abonat ja veu l'error a l'app en aquell moment | E5-T10 | N-40 parla d'un pagament caducat, que no és el que ha passat |
| E31 | Alineacions de catàleg i de spec proposades per l'executora: N-16 variable `audience` (la còpia dels admins no parla de cap gos) · N-46 només a qui va rebre l'N-15 de l'oferta, i també quan una reserva `PAY_TO_BOOK` ocupa la plaça · `UpfrontPayment*` porten `memberId`, `amountPaid` i `bookingId` (reserves) · col·lecció tècnica `ring_day_locks` al model (substituïda per `ring_slot_locks`, un document per slot de pista, a E5-T07 ronda 2) · la targeta de risc de D1 és el formulari A de `/risk-review` (S14 R-14-06 remet a S15 §6) · l'API pública amb clau respon `403 INVALID_API_KEY` abans de dir si el club existeix (S07) | catàlegs, S14, S07 i model (aplicat) | coherents amb les specs de detall; no canvien res del que hagis de decidir |
| E32 | Migració, cas 5 (B33) amb les dades reals: de les 20 adreces repetides, **17 són la mateixa persona amb dos gossos** (Playoff obliga a fer una fitxa per gos, i la segona porta un altre NIF, sovint començat per 0) i **3 són famílies**. La migració uneix en **una sola persona amb diversos gossos** les fitxes que el club confirmi al llistat (fitxer auxiliar `persones.csv`: parelles d'«ID associat» + NIF bo); les famílies segueixen la proposta (compte per a la fitxa d'alta més antiga; l'altra, sense compte i amb proposta de grup familiar). A més, **només els abonats d'alta reben compte**: les baixes migrades es queden el correu com a contacte, perquè 11 adreces d'alta també surten en baixes d'altres persones | S18 R-18-04 (f), R-18-12, T-18-06 (aplicat) · codi: E5-T12 | sense això, la migració crearia 17 persones duplicades sense compte |
| E33 | Primer desplegament dels processos automàtics: la primera execució d'un procés en un club sense cap execució prèvia, si cau fora de finestra, es registra com a «omesa» **sense alerta** (N-42) | S15 R-15-05 (aplicat) · codi: E5-T13 | si no, el dia del release cada club rebria alertes falses de processos «perduts» |
| E34 | Un pagament que el proveïdor completa **després** que el procés P7 hagi caducat la reserva queda registrat i es **reemborsa** (S12, E8), en comptes de perdre's | S15 R-15-17 · codi: E5-T13 (registre) i E8-T04 (reemborsament) | P7 caduca la sessió al nostre costat, però Stripe pot haver cobrat igualment |
| E35 | El gràfic «Gossos per nivell» de D1 fa servir els mateixos nivells que la cobertura de D3: una columna per nivell actiu **de la progressió** (E29). Els gossos dels altres nivells (Teràpia, Pendent, nivells inactius) compten a «altres», que no es pinta | S14 R-14-07 (aplicat) · codi: E3-T08 (api), E3-W07 (web) | el tauler i D3 han de comptar igual; amb E29, Teràpia i Pendent no són nivells de progressió |
| E36 | La pantalla 13 també mostra els gossos propis **pendents de validació**, amb el xip «pendent de validació» i sense accions; `GET /me/dogs` els retorna amb `status` | S03 fila 13 i R-03-02, S04 R-04-25 (aplicat) · codi: E3-T08 (api), E3-W08 (web) | després d'afegir un gos, l'abonat no en veia cap rastre fins que el club el validava |
...
| E38 | Una **readmissió** (algú que torna amb el DNI d'un abonat de baixa) ja no sobreescriu la fitxa en enviar el formulari: les dades noves esperen a la sol·licitud, D2 mostra el valor antic i el nou, i s'apliquen en validar. Si es rebutja, la fitxa queda exactament com era (baixa amb el seu motiu i la seva data originals). **Inclou el gos reutilitzat** (el xip d'un gos `INACTIVE` de l'abonat), amb els seus documents: les dades i els documents enviats també esperen a la sol·licitud, i un rebuig el deixa `INACTIVE` amb el motiu, la data, les dades i els documents originals (organitzador 25-09, re-execució de la porta E3) | S04 R-04-06 i R-04-23 (aplicat; el gos, 25-09) · codi: E3-T09 (api), E3-W07 (web), E3-T17 (api, el gos) | qualsevol que sabés el DNI d'un exabonat li podia reescriure la fitxa, i un rebuig en perdia les dades originals (RGPD art. 5) |
...
| 09-09 (Jordi) | **A30**: el programari es queda **preparat** per a Learn (S19/ADR-010 intactes) però **no s'hi vincula** fins a passar d'un sol club al SAAS; no es carrega la BBDD de producció de Learn. E1-T08 sense data, porta E1 alliberada |
| 09-09 (Jordi) | **A28 i A29 tancades**: no es tria una operativa, es tria un **valor per club**. Les dues branques (`billing.sepa.collectionDayOfMonth` `1–28`/`0` i `billing.cashInvoicing` `MONTHLY`/`SEMESTER`) han de ser de producte, provades i fàcils de configurar des de D11; el Cànic manté `1` i `SEMESTER`. Criteri declarat: **«cada club és un món»** |

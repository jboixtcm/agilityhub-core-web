...
| escala AgilityHub | `AgilityHubLevel` (enum/catàleg global) | platform | no |
...
| **NIVELL** (`Level`) | `name: LocalizedText` (fallback al text pla), `order`, `color`, `capacity` (aforament), `active`, **`grantsFreeTraining`** (bool; Cànic: D..G), **`agilityhubLevel`** (opcional, escala global = la dificultat real de Learn: `EASY` · `MEDIUM` · `HARD` — verificat al codi de Learn el 05-09) | ADR-002, decisió 03-09 |
...
| `Dog` | `status PENDING · ACTIVE · INACTIVE` + `deactivationReason (CLUB · MEMBER_LEFT · SIGNUP_REJECTED)`, `levelAssignedAt`, `levelHistory[]`, `instructorNote` (de l'alumne), `remarks` + `remarksMeta` (observacions privades), `photoFileKey`, `licenses[] {organisation, number, grade}`, `trainingSeq`, `sourceIds` | S03, S04, S09, S10, S18 |
...
| `Level` · `Ring` | `name: LocalizedText`, `order`, `color`, `capacity`, `active`, `grantsFreeTraining`, `agilityhubLevel` · `shortName`, `color`, `order`, `allowsFreeTraining`, `trainingCapacity`, `active`, `geometry?`, `activeSetupId` | S05, S16 |

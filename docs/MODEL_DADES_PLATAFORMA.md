...
| decisió del límit de correus per destinatari (nou, 25-09) | `signup_notification_admissions` `{_id: eventId:code, clubId, eventId, notificationCode, admitted, decidedAt, expiresAt}`, sense cap dada del destinatari. Es desa abans de consumir el límit (S04 R-04-20, E3-T15). No té `expiresAt` mentre l'esdeveniment es pot reintentar; després, `expiresAt` = processat + `jobs.retention.domainEventsDays` (TTL). Si la caducitat posterior al commit falla, o l'esdeveniment acaba `FAILED`, la decisió es queda sense `expiresAt` (acceptat: no té dades personals) | clubs/census | sí |
| bloqueig tècnic pista-slot (nou, 24-09) | `ring_slot_locks` `{_id: clubId:ringId:startsAt, clubId, ringId, startsAt, sequence, expiresAt}`: el toquen (`$inc`) una reserva d'entrenament (el seu slot) i cada escriptura que comprova les reserves de la pista: S05 desactivar la pista o treure-li `allowsFreeTraining` (tots els slots de la finestra de reserva), S06 bloqueig, classe moguda i validació de la setmana (els slots de les seves classes DRAFT futures, E5-T15) i S07 bloqueig d'activitat (els slots de la graella que el rang solapa) (S09 R-09-13). TTL: `expiresAt` = `startsAt` + 7 dies (S15 R-15-19, 25-09; E5-T17). Substitueix `ring_day_locks`, que concentrava les reserves d'un dia en un sol document (E5-T07 ronda 2) | clubs/scheduling | sí |
...
| **NIVELL** (`Level`) | `name: LocalizedText` (fallback al text pla), `order`, `color`, `capacity` (aforament), `active`, **`grantsFreeTraining`** (bool; Cànic: D..G), **`progression`** (bool, per defecte sí; Teràpia i Pendent no — E29) — **sense** `agilityhubLevel` (Jordi 06-09, A5: els nivells de club són catàleg 100 % local; `EASY·MEDIUM·HARD` només a `Course`/`RingSetup`/`Challenge`) | ADR-002, decisió 03-09 |
...
- Índexs compostos obligatoris `{clubId, …}` a totes les col·leccions de club; `members {clubId, contactEmails.email}` i `{clubId, readmissionRequest.submitted.contactEmails.email}` (R-04-05) i `dogs` amb una clau de nom normalitzada (R-04-12) (24-09, E3-T09); `accounts.email` únic; `memberships {accountId, clubId}` únic; `seat_holds.expiresAt` TTL; `magic_link_tokens.expiresAt` TTL; `stripe_events.eventId` únic; `invoices {clubId, series, number}` únic; `bookings {clubId, classSessionId, dogId, state}`; `training_bookings {clubId, slotId}`; `ring_setups {clubId, ringId, status}`.
...
| `Member` | `number`, `status PENDING · ACTIVE · LEFT` (+ `displayStatus` derivat: `PENDING · ACTIVE · INACTIVE_PERIOD · LEAVE_SCHEDULED · LEFT`), `leftAt`, `leftReason (SIGNUP_REJECTED · LEAVE_REQUEST · ADMIN · PACK_EXPIRED · MIGRATED)`, `leaveDate`, `leaveRequestId`, `leaveHistory[]`, `billingMode (MONTHLY_FEE · MAINTENANCE)`, `contactEmails[] {email, bounced}`, `phones[] {number, label}`, `consents[] {type, version, granted, acceptedAt, ipHash, source, origin, actorAccountId}` (`origin`/`actorAccountId`: qui l'accepta, també sota impersonació; 24-09, E3-T09), `signup {…}` (només l'alta pública, `source = PUBLIC`; a més dels camps de S04 §3: `ipHash`, `userAgent`, `paymentMethodTypeRequested`, `upfront.firstMonth {option, startDate, amountDue}`, `validatedAt/ByAccountId`, `rejectedAt/ByAccountId`, `rejectionReason`; 25-09, E3-T10), `readmissionRequest {submitted {…}, previous {status, leftAt, leftReason, leaveDate, familyGroupClaim, signup}}` (readmissió pendent, E38; 24-09, E3-T09), `familyGroupClaim`, `internalNotes`, `remarks`, `trainingSeq` (tècnic), `sourceIds {playoffMemberId, playoffNumber}` · `billedViaMemberId` **derivat** (= titular del grup) | S03, S04, S12, S13, S18 |
| `Dog` | `status PENDING · ACTIVE · INACTIVE` + `deactivationReason (CLUB · MEMBER_LEFT · SIGNUP_REJECTED)`, `levelAssignedAt`, `levelHistory[]`, `instructorNote` (de l'alumne), `remarks` + `remarksMeta` (observacions privades), `photoFileKey`, `licenses[] {organisation, number, category?, grade?, division?}` (09-09: `category` i `division` venen de Playoff), **`handlerName`** (09-09: nom del guia quan no és l'abonat; informatiu, sense compte ni permisos), `signup {…}` (25-09, E3-T10: el bloc de la sol·licitud que va crear el gos; una alta pública hi copia `Member.signup`; un gos afegit hi té el seu propi, amb `submissionId`, `locale`, `planIdRequested`, `upfront` congelat i `additionalDogOption`, i la decisió de cada gos s'hi segella), `readmissionRequest {submitted {name, sex, breed, birthDate, instructorNote, documents[{type, files[]}]}, previous {status, deactivatedAt, deactivationReason, signup}}` (el gos reutilitzat d'una readmissió pendent, E38; 26-09, E3-T17), `trainingSeq`, `sourceIds` (`playoffMemberId`, `playoffDogIndex` i, des del 24-09, `playoffPhoto`: la referència de la foto a Playoff que `migration:photos` baixa a `photoFileKey`, S18) | S03, S04, S09, S10, S18 |
...
| `Level` · `Ring` | `name: LocalizedText`, `order`, `color`, `capacity`, `active`, `grantsFreeTraining`, `progression` · `shortName`, `color`, `order`, `allowsFreeTraining`, `trainingCapacity`, `active`, `geometry?`, `activeSetupId` | S05, S16 |
...
| `TrainingBooking` · `Attendance` | `ringId`, `startsAt`, `endsAt`, `seatIndex`, `weekStart`, `state`, `cancelReason`, `origin` (índex únic parcial `{clubId, ringId, startsAt, seatIndex}` sobre `ACTIVE`; els slots es calculen) · `bookingId`, `state PENDING · PRESENT · NOTIFIED · NO_SHOW`, `markedBy {accountId, role, displayName}`, `markedAt`, `notice {at, late, minutesBefore, seatReleased, waitlistNotified, afterClassEnd, bookingState}`, `noShowNotice {queuedAt, eventId, sentAt}` (índex `{clubId, state, "noShowNotice.queuedAt", classDate}`; abans `markedByAccountId`, `late` i `noticeSentAt`: E6-T01, 24-09) | S09, S10 |

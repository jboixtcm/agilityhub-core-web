...
| 2 | ~~Canvis de l'abonat sobre un període ja aprovat/actiu.~~ **Resolt (Josep 08-09): s'apliquen sols** dins la regla del dia 25, amb avís N-18d als admins. | — | — |
| 3 | ~~Quota d'inactivitat per a plans `PACK`/`SINGLE_CLASS` i si el pack es pausa.~~ **Resolt (Josep 08-09)**: els plans `PACK` **no poden demanar inactivitat** (`422 INACTIVITY_NOT_APPLICABLE`, R-13-02); el pack no es pausa mai i venç a la seva data. | — | — |
...
| 8 | ~~Reactivació des de D10 (R-13-16): entrada de nou?~~ **Resolt (Josep 08-09): sense entrada** en reactivar després d'una inactivitat; l'admin pot registrar un cobrament anticipat si cal. Gossos un a un (S03 §13-6). Nota: passar **de pack a abonat** sí que paga entrada, amb el 40 % de descompte de S05 R-05-18b. | — | — |

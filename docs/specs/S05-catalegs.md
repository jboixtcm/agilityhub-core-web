...
| **R-05-07** Desactivar una pista | Bloquejat (`409 RING_IN_USE`) mentre tingui classes futures no anul·lades; amb reserves d'entrenament vives segueix R-05-08 (`422 RING_HAS_BOOKINGS` llevat de `cancelBookings: true`; organitzador 24-09); els bloquejos futurs i les classes de plantilla **no** bloquegen (S06 les marca inconsistents; S09 anul·la els slots lliures futurs de la pista). | — | «Petita» amb 1 classe dissabte vinent → 409 `{futureClassSessions: 1}`; sense classes → 200 i S09 retira els slots. |
| **R-05-08** «Reservable per entrenaments» | Amb `allowsFreeTraining = false` la pista no genera slots (S09). Passar de sí a no amb reserves d'entrenament vives segueix **S09 R-09-13**: `422 RING_HAS_BOOKINGS {bookings[]}` (l'estat del catàleg d'errors, regla 0; corregit el 24-09) llevat que l'ADMIN enviï `cancelBookings: true` (cada reserva → `CANCELLED_BY_CLUB` / `RING_NOT_RESERVABLE`, dins la mateixa transacció); sense reserves, els slots lliures futurs desapareixen (organitzador 24-09: mana S09). Amb `FREE_TRAINING` off el camp s'accepta però no té efecte. | `FREE_TRAINING` | Cànic: Cadells i Petita = no. |
...
| **R-05-21** Lectura pública | `GET /public/{clubSlug}/plans` retorna només modalitats `active && showOnWeb`, amb textos resolts per `Accept-Language ∩ club.locales` (fallback `defaultLocale`) **i** els mapes `*I18n`; preus i entrada només si `BILLING` on; mai cap dada personal. Cal `X-Api-Key` = clau pública del club (S02) → `403 INVALID_API_KEY`, que es comprova **abans** que el club: un slug inexistent sense clau vàlida també és 403 (organitzador 24-09, com S07 R-07-12). `Cache-Control: public, max-age=300`; rate limit per IP. | `BILLING` | Web del Cànic en castellà: «Abonado · 60 €/mes». |
...
| GET | `/public/{clubSlug}/pages/{key}` | ANON (`X-Api-Key`) | — | sí | pàgina publicada (web del club, enllaç de l'alta 19 «Normes del club») | `Accept-Language` | 200 · 403 `INVALID_API_KEY` (clau primer) · 404 pàgina inexistent o inactiva |
| GET | `/faq-entries/filter-values?field=category` | ADMIN | `FAQ` | sí | categories existents amb recompte (§4 de les convencions) | | 200 |
| GET | `/public/{clubSlug}/plans` | ANON | `BILLING` per als preus | sí | R-05-21 | header `X-Api-Key`, `Accept-Language` | 200 `{club: {slug, name, currency}, plans[]}` · 403 `INVALID_API_KEY` (també amb un slug inexistent) · 403 `CLUB_SUSPENDED` (només amb la clau del club) · 429 |
...
- T-05-12 (R-05-07/08): pista amb classe futura → desactivar 409; amb reserva d'entrenament futura → `allowsFreeTraining=false` o desactivar → 422 `RING_HAS_BOOKINGS`; sense res → 200 i `RingChanged` a l'outbox amb el `diff`.
...
- T-05-18 (R-05-21): `GET /public/canic/plans` sense clau → 403 `INVALID_API_KEY`; amb clau i `Accept-Language: es` → noms en `es`, només `showOnWeb`, sense `memberId`/dades personals; `BILLING` off → sense `price`/`entryFee`; slug inexistent → 403 `INVALID_API_KEY` (la clau es comprova abans que el club).
...
- T-05-28 (R-05-08) treure «Reservable per entrenaments» a una pista amb reserves d'entrenament futures → `422 RING_HAS_BOOKINGS{bookings}`; amb `cancelBookings: true` → reserves `CANCELLED_BY_CLUB/RING_NOT_RESERVABLE`; sense → OK i `GET /training-slots` (S09) ja no la llista.
...
- 26-09-2026 · verificació d'E4-W06: l'ajuda de l'interruptor «Progressió» no anomena cap nivell del club (la d'abans deia «Teràpia» i «D»).

...
| `pack` | `{sessions, validityMonths}` | si `PACK` | 1–99 · 1–24 | caducitat → «baixa prevista» automàtica (S12/S15) |
...
| **R-05-04** Ordre | `PUT /{levels,rings,faq-entries}/order` rep **tots** els ids del club (actius i inactius) sense repeticions; `order = índex × 10`. Ids que faltin o sobrin → `400 ORDER_INCOMPLETE`. | — | `[CAD, A, B, C, D, E, F, G, TER]` → 0,10,…,80. |
...
| PUT | `/levels/order` | ADMIN | — | sí | reordena | `{levelIds[]}` | 200 · 400 `ORDER_INCOMPLETE` |
...
- T-05-27 (R-05-03) desactivar un nivell amb gossos actius → `409 LEVEL_IN_USE`; sense gossos ni classes futures → `INACTIVE` i desapareix dels selectors però es manté a l'històric.

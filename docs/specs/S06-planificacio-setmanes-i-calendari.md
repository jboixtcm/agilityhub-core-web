...
**R-06-03 · Descripció automàtica i manual.** `description` guarda **només** el text manual. `displayDescription` = `description` si no és buit; si no, text automàtic a partir de `levelIds` ordenats per `Level.order` i amb `Level.name` resolt a l'idioma del lector: 1 nivell → «{nom}»; conjunt **contigu** de nivells de la progressió (`Level.progression`, S05) que arriba a l'últim nivell actiu de la progressió i té ≥ 2 nivells → «{primer} i sup.» (`es` «{primer} y sup.», `en` «{primer} and up»); la contigüitat es compta sobre els nivells actius de la progressió ordenats per `order`; qualsevol altre conjunt, també si inclou un nivell fora de la progressió → noms units amb «+» (E29, 24-09). Sense nivells (`levels.enabled=false`) el text manual és obligatori (422 `DESCRIPTION_REQUIRED`). Esborrar el text manual (`description: null`) torna a l'automàtic. La classe generada copia `description` (manual) i `levelIds`; el text automàtic es recalcula sempre en llegir. *Exemple* (catàleg Cadells·A·B·C·D·E·F·G·Teràpia, Teràpia fora de la progressió): {B,C} → «B+C» · {C,D,E} → «C+D+E» · {D,E,F,G} → «D i sup.» · {A,C} → «A+C» · {Cadells} → «Cadells» · {G,Teràpia} → «G+Teràpia» · {B,C} amb manual «Obed. urbana» → «Obed. urbana» fins que s'esborri.
...
**R-06-06 · Cobertura per nivell** (càlcul, mai desat; només si `levels.enabled`). Per a cada nivell actiu **de la progressió** L (`Level.progression`, S05; ordre del catàleg, Cadells inclòs; els nivells de fora, com Teràpia o Pendent, no hi surten — E29) sobre el conjunt de classes C (plantilla: classes de la plantilla dl–dv visible + de la de dissabtes visible; setmana: `ClassSession` no anul·lades):
...
- T-06-02 (R-06-03) Given catàleg Cadells…G + Teràpia (fora de la progressió) Then {B,C}→«B+C», {D,E,F,G}→«D i sup.», {G,Teràpia}→«G+Teràpia», {A,C}→«A+C», {Cadells}→«Cadells»; manual «Obed. urbana» preval; `null` restaura l'automàtic; en `es` «D y sup.», en `en` «D and up».
...
- 16-09-2026 · obertura d'E4 (E4-T01…T05, E4-W01…W05): estats HTTP segons `CATALEG_ERRORS.md` (mana el catàleg): `BAND_OVERLAP` i `RING_BLOCK_CONFLICT` **409**, `WEEK_ALREADY_GENERATED` i `BAND_NOT_EMPTY` **409** (afegits explícitament a §1), `LEVELS_DISABLED` **422** (no 404), `TEMPLATE_INCONSISTENT` / `WEEK_INCONSISTENT` / `NOTHING_TO_VALIDATE` / `RING_HAS_BOOKINGS` / `RING_BLOCK_MANAGED_BY_ACTIVITY` / `RING_BLOCKED` **422** (no 409). `RING_BLOCKED` és l'error de crear/moure a mà una classe sobre una pista bloquejada; en generar és una incoherència. Auditoria: `TEMPLATE_BAND_DELETED` afegida a S14 R-14-09 (A22 b). Els noms de fil són els de S06 §6 (`bands`, `counters.waiting`); el model persisteix `timeBands`/`risk{}`. Ruta de la pantalla 23 al front: `/instructor/avui` (registrada a PLA_FRONTEND), no `/instructor/visio-global`.

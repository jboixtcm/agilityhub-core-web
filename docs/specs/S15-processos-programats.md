...
| `Attendance` (S10) | `noShowNotice.queuedAt` (abans `noticeSentAt`; E6-T01, 24-09) | P3 |
...
| Abast | `Attendance` amb `state=NO_SHOW`, `noShowNotice.queuedAt == null` i data local de la classe `< today` (índex `{clubId, state, "noShowNotice.queuedAt", classDate}`) — inclou marques posades tard (un instructor que passa llista dos dies després) |
| Regla | un sol lot: `NoShowNoticeDue{attendanceIds[], bookingIds[]}` en una transacció que fixa `noShowNotice.queuedAt=now` (i `eventId`) a tots; el dispatcher envia N-19 per reserva. Una marca canviada a `PRESENT`/`NOTIFIED` abans del lot no s'avisa; canviar-la després no retira res. |
...
| Idempotència | `Attendance.noShowNotice.queuedAt` |
...
**Contractes que aquest vertical necessita**: S06 `ClassSessionRepository.findActiveBetween`, `ClassCancellationUseCase.cancel(RISK_REVIEW)`, `finishEnded(now)`, `RiskEvaluator`; S08 `BookingCancellationService.cancel(by=SYSTEM)`, `cancelFutureByMember`, `WaitlistService.offerNext`, `sweepStarted`, `cancelByMember`, cache base de classes reservables; S09 `TrainingBookingService.cancelFutureByMember`; S10 `Attendance` amb `noShowNotice.queuedAt`; S12 `PackBalance` amb `expiresOn`; S13 `InactivityPeriod`, `Member.leaveDate`, consumidor de `PackExpired`; S16 `RingSetup.expiresAt`; S02 `ClubConfig` i escriptura de paràmetres; S11 dispatcher amb fan-out per lots.
...
| N-42 «Procés automàtic amb errors» (proposta) | R-15-10, màx. 1 per procés i dia | ADMINS → APP (+EMAIL); acció `OPEN_JOBS` (proposta: obre D11#processos) | `job_name`, `error_count` (sense `date`: organitzador 24-09, mana el catàleg) |
...
- T-15-16 (R-15-13) 3 `NO_SHOW` d'ahir + 1 de fa 3 dies sense avís + 1 ja avisada + 1 canviada a `PRESENT` a les 22:00 → un `NoShowNoticeDue` amb 4 ids, `noShowNotice.queuedAt` fixat, N-19 ×4 segons preferències (`late=1`); segona execució → 0.

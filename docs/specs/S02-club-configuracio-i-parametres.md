...
| **R-02-02 `/branding`** | `GET /branding` (ANON, per host) retorna el subconjunt públic de `ClubConfig`: `{clubId, slug, name, legalName, taxId, locales, defaultLocale, timeZone, currency, countryProfile: {idDocumentTypes, postalCodeLookup, phonePrefix, requiresIban, mandateTextKey, taxLabelKey}, theme, pwa, modules, signup: {enabled}, legal: {privacyPolicyUrl}, app}`; cache HTTP 60 s (`ETag`). `legalName` i `taxId` són identificadors públics de l'entitat, i la LSSI (art. 10) els demana a la web: el peu públic els mostra (organitzador 24-09). Mai paràmetres de negoci sensibles ni secrets. | `signup.enabled` | La PWA pinta el tema i amaga «Entrenaments» si `FREE_TRAINING` no hi és. |

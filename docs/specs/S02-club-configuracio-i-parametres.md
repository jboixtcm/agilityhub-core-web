...
| | `name`, `legalName`, `taxId`, `address {street, postalCode, city, region, country}` (el domicili), `displayCity` (la població que es mostra amb el nom del club, per exemple on entrena; per defecte `address.city`), `contactEmail`, `contactPhone`, `websiteUrl` | | `name` sí | `taxId` validat pel perfil de país quan canvia, i desat normalitzat (majúscules, sense espais ni separadors; el perfil `ES` completa un DNI de 7 xifres fins a 8): el mateix identificador escrit d'una altra manera no és cap canvi. Un valor que només té separadors (`"-"`) es refusa (`400 VALIDATION_ERROR`, `taxId`); un valor en blanc (buit o només espais) o `null` l'esborren (`null` només amb `PUT /club`: l'esquema de la definició del club el refusa, i la definició l'esborra amb `""`) |
...
| **R-02-02 `/branding`** | `GET /branding` (ANON, per host) retorna el subconjunt públic de `ClubConfig`: `{clubId, slug, name, legalName, taxId, city, legalAddress, locales, defaultLocale, timeZone, currency, countryProfile: {idDocumentTypes, postalCodeLookup, phonePrefix, requiresIban, mandateTextKey, taxLabelKey}, theme, pwa, modules, signup: {enabled}, legal: {privacyPolicyUrl}, app}`; cache HTTP 60 s (`ETag`). `legalName` i `taxId` són identificadors públics de l'entitat, i la LSSI (art. 10) els demana a la web: el peu públic els mostra (organitzador 24-09). `city` = `displayCity ?? address.city`; `legalAddress {street, postalCode, city}` és el domicili (`address`), o `null` si no té carrer o codi postal. El peu públic diu «{legalName} · {taxId} · {city}» i, a sota, el domicili «{street} · {postalCode} {city}» (Jordi, 25-09). Mai paràmetres de negoci sensibles ni secrets. | `signup.enabled` | La PWA pinta el tema i amaga «Entrenaments» si `FREE_TRAINING` no hi és. |
...
| **R-02-05 Paràmetres de temps** | `time` i `weekOpensAt` són **hora local** del club; la UI mostra el fus («Diumenge 20:00 · Europe/Madrid»). Canviar `club.timeZone` (S17) és una operació de plataforma que exigeix que no hi hagi classes futures generades (`422 TIMEZONE_CHANGE_BLOCKED`) — les dates de negoci es guarden com a locals, els instants en UTC. | `club.timeZone` | — |
...
- T-02-07 (R-02-02) `/branding` del Cànic conté tema, mòduls, `countryProfile.idDocumentTypes = [DNI, NIE, PASSPORT]`, `signup.enabled`, `city` «Cabrera de Mar» i `legalAddress` amb el domicili; sense cap secret ni paràmetre de negoci; `ETag` i `304`.
...
- T-02-15 (R-02-05) `PUT /parameters/bookings.weekOpensAt {SUNDAY, 20:00}` en un club `America/Argentina/Buenos_Aires` → l'obertura (S15) cau a les 20:00 locals (23:00Z); `PATCH /platform/clubs/{id} {timeZone}` amb classes futures → `422 TIMEZONE_CHANGE_BLOCKED`.
...
- 26-09-2026 · verificació d'E5-T17: §3 diu que el perfil `ES` desa un DNI de 7 xifres completat fins a 8, i que un `taxId` en blanc (buit o només espais) l'esborra, com `null`.
- 26-09-2026 · verificació d'E5-T18: §3 diu que una definició de club esborra el `taxId` amb `""` (el seu esquema refusa `null`); `null` l'esborra només amb `PUT /club`.

...
| 29 `29-confirmar-reserva-estats.html` | `apps/clubs` | `/reservar/confirmar` | MEMBER | Mateixa ruta, quatre variants: normal → «CONFIRMAR LA RESERVA» (+ compte enrere) → `POST /bookings {seatHoldId}`; `409 BOOKING_LIMIT_REACHED` sense `swappable` → nota groga. La primera frase segueix `details.week` i `details.unit`, com la nota de l'intercanvi. La segona depèn de la setmana: amb `CURRENT`, «Podràs reservar per a la setmana vinent a partir de diumenge {dia} a les 20 h.» (el literal del mockup, amb la data de `nextBookableAt`); amb `NEXT`, «Podràs reservar aquesta classe a partir de diumenge {dia} a les 20 h.» (B1: a `nextBookableAt` la classe passa a la setmana en curs). Amb `unit = MEMBER`, sense «amb la Duna». Exemple amb `CURRENT`: «Aquesta setmana ja has fet dues classes amb la Duna. Podràs reservar per a la setmana vinent a partir de diumenge 9 a les 20 h.» (organitzador 26-09, revisió d'E5-W01: amb `CURRENT`, la classe ja haurà passat a `nextBookableAt`); `409 NOT_YET_OPEN` → nota grisa «Disponible a partir de diumenge 24 a les 20 h.»; compte enrere a 0 o `409 SEAT_HOLD_EXPIRED` → nota vermella «Reserva cancel·lada per temps. Toca per tornar a la llista de classes.» (tocar-la → 04). Amb `SINGLE_CLASS`: `PAY_TO_BOOK` → botó «PAGAR I CONFIRMAR ({preu})» i redirecció a Checkout; `CHARGE_ON_ATTENDANCE` → nota «Aquesta classe es carregarà al proper rebut ({preu}).» (assumpcions). |
...
**R-08-10 Anul·lació.** `POST /bookings/{id}/cancellation` sempre possible mentre `state=ACTIVE`, `now < classEndsAt` **i** no existeix cap `Attendance` `PRESENT`/`NO_SHOW` de la reserva (contracte S10 R-10-03; `422 BOOKING_NOT_CANCELLABLE` altrament, com diu el catàleg; organitzador 26-09). `minutesBefore = (classStartsAt − now)` en minuts (pot ser negatiu). `late = now > classStartsAt − bookings.lateCancelThresholdMinutes` (llindar **inclòs** com a dins termini). `late=false` → `CANCELLED` (no compta, pack retornat, política `billing.singleClassCancelPolicy` si estava pagada); `late=true` → `CANCELLED_LATE` (compta, sense retorn). Qui: el membre (propi o del grup), l'admin «com l'abonat» (`origin=BACKOFFICE`), l'instructor si `bookings.instructorLastMinuteNotice=true` (`origin=INSTRUCTOR`, `reason=INSTRUCTOR_NOTICE`), el sistema (S13/S15, sempre `CANCELLED`). L'admin sense impersonar → `403`. Paràmetre `bookings.lateCancelThresholdMinutes = 240` (4 h, Jordi 05-09; el catàleg). *Exemples (classe dj 15-10 18:50):* anul·lació a les 16:50:00 → dins termini; 16:50:01 → tard; 19:05 (classe començada) → tard, `minutesBefore = −15`.
...
| POST | `/bookings/{id}/cancellation` | MEMBER (pròpia/grup) · INSTRUCTOR (si paràmetre) | — | — | Anul·la (R-08-10) | `{message?}` | 200 `Booking{state, late, minutesBefore}` · 403 · 422 `BOOKING_NOT_CANCELLABLE` |
...
- 08-09-2026 · respostes del Josep (B11): R-08-17 — el retorn d'una sessió a un pack ja caducat no el reactiva; la sessió es perd.
- 26-09-2026 · verificació d'E5-W01: la nota de límit de 29 segons `details.week` (amb `CURRENT`, el literal del mockup; amb `NEXT`, el de B1) i `details.unit`; R-08-08, `Idempotency-Key` un UUID per cos, no el `seatHoldId`; R-08-10 i §6, `BOOKING_NOT_CANCELLABLE` és 422 (el catàleg). Verificació d'E5-T25: R-08-10 diu el valor del catàleg, 240 (deia 120).

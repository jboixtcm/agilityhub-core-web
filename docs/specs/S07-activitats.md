...
| GET | `/activities/{id}/ring-conflicts` | ADMIN | ACTIVITIES | sí | vista prèvia del diàleg de R-07-05 | — | 200 `{conflicts[], trainingBookings[]}`; 422 `OUTSIDE_OPENING_HOURS` · 400 `INVALID_TIME_RANGE` (la mateixa finestra que la publicació refusaria) |
...
| GET | `/public/{clubSlug}/activities` · `…/activities/{slug}` | ANON | ACTIVITIES | sí | R-07-12 | `X-Api-Key`, `Accept-Language`, `?scope=upcoming\|past` | 200 Forma C; 403 `INVALID_API_KEY` (abans que el club i el mòdul); 404; 429 |
...
| 2 | ~~Termini d'anul·lació per l'abonat~~ **Resolt (Josep 08-09): fins a l'hora d'inici de l'activitat** | — | `activities.cancelDeadline = EVENT_START` |
...
- 08-09-2026 · respostes del Josep (B20): `activities.cancelDeadline = EVENT_START` al Cànic (l'inscrit pot anul·lar fins a l'hora d'inici).
- 16-09-2026 · obertura d'E4: estats HTTP segons `CATALEG_ERRORS.md`: `ACTIVITY_FULL` i `DUPLICATE_SLUG` **409** (afegits a §1 / prefix `DUPLICATE_`), `SLUG_LOCKED` i `ALREADY_REGISTERED` 409, `INVALID_API_KEY` **403** (no 401), `LOCALE_NOT_ENABLED` **422** (no 400), `REGISTRATION_CLOSED` / `ACTIVITY_NOT_PUBLISHED` / `ACTIVITY_HAS_REGISTRATIONS` / `REGISTRATION_NOT_CANCELLABLE` / `LEVEL_NOT_ALLOWED` **422** (no 409). L'exportació d'inscrits manté la ruta ja publicada `GET /activity-registrations/export` (S14, E2-T01) amb `filter=activityId:eq:`, no `/activities/{id}/registrations/export`. Auditoria: `ACTIVITY_UPDATED`, `ACTIVITY_REGISTERED_BY_CLUB`, `ACTIVITY_REGISTRATION_CANCELLED_BY_CLUB` afegides a S14 R-14-09. Seed demo: «Seminari de handling» se sembra ple (12/12 + 2 en espera) per provar la promoció FIFO.
- 24-09-2026 · `INVALID_API_KEY` és **403** també a R-07-12, §6 i T-07-17 (el catàleg mana; ja ho deia el canvi del 16-09) i la clau es comprova abans que el club i el mòdul (verificació d'E5-T08).
- 24-09-2026 · verificació d'E4-W04: (1) estats segons la regla 0 de `CATALEG_ERRORS.md`: `RING_HAS_BOOKINGS`, `REGISTRATION_CLOSED`, `ACTIVITY_NOT_PUBLISHED`, `ACTIVITY_HAS_REGISTRATIONS`, `REGISTRATION_NOT_CANCELLABLE` i `RING_BLOCK_MANAGED_BY_ACTIVITY` són **422**; els «409» d'aquest text per a aquests codis s'han de llegir amb el catàleg (`STALE_VERSION`, `INVALID_STATE`, `DUPLICATE_SLUG`, `SLUG_LOCKED`, `RING_BLOCK_CONFLICT` i `ACTIVITY_FULL` continuen 409). (2) El 07-08-2026 dels exemples és **divendres**: la UI mostra «dv 7»; el «ds 7» del mockup és il·lustratiu. (3) Els models de lectura (`ActivityListItem`, `RegisteredActivity`, files de 03/25) envien `endTime: null` per a una activitat sense hora de final, en lloc del conveni «l'endemà a les 00:00» del model (api E4-T06).
- 26-09-2026 · verificació d'E4-W05 i E4-W11: `ring-conflicts` declara 422 `OUTSIDE_OPENING_HOURS` i 400 `INVALID_TIME_RANGE`; les inscripcions en llista d'espera porten `waitlistRank` (el rang entre les actives; `position` es manté com a ordre desat) i D7 i l'app el mostren; `appliedFilters` del llistat d'inscrits no hi posa l'`activityId` del camí (E5-T20).

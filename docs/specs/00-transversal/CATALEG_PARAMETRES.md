...
| `signup.text.paymentDay` | localizedText | `{"ca": "Quan ja no vulguis seguir fent activitats al club o vulguis estar un temps sense venir, indica'ns-ho abans del dia {deadlineDay} per modificar l'emissió de rebuts del mes següent. Ho pots fer des de la teva fitxa o bé contactant-nos directament.", "es": "Cuando ya no quieras seguir haciendo actividades en el club o quieras estar un tiempo sin venir, indícanoslo antes del día {deadlineDay} para modificar la emisión de recibos del mes siguiente. Puedes hacerlo desde tu ficha o contactando con nosotros directamente.", "en": "When you no longer want to train at the club, or you want to take a break, let us know before day {deadlineDay} so we can change the following month's billing. You can do it from your profile or by contacting us."}` | pantalla 19 · text literal del Josep (mockup 19 V8); la frase del dia de càrrec viu a `signup.text.monthlyPaymentIntro` i ha de concordar amb `billing.sepa.collectionDayOfMonth` |
| `signup.text.cashConditions` | localizedText | `{"ca": "Pagament en efectiu: la quota es paga per semestres naturals complets al club. Posa't en contacte amb el club per coordinar-ho.", "es": "Pago en efectivo: la cuota se paga por semestres naturales completos en el club. Ponte en contacto con el club para coordinarlo.", "en": "Cash payment: the fee is paid at the club in full calendar half-years. Contact the club to arrange this."}` | 19 · ha de concordar amb `billing.cashInvoicing` (aquest text és el cas `SEMESTER`, mockup 19 V4/V5) |
...
| `signup.text.monthlyPaymentIntro` | localizedText | `{"ca": "La quota mensual per abonats es cobrarà normalment el dia 1. En el cas de packs no es genera cap càrrec.", "es": "La cuota mensual para abonados se cobrará normalmente el día 1. En el caso de packs no se genera ningún cargo.", "en": "The monthly membership fee is normally collected on the 1st. Packs do not generate a monthly charge."}` | Alta i consentiments | S04 · mockup 19 V6, text del Josep 23-08; **ha de concordar amb `billing.sepa.collectionDayOfMonth`** |
...
| `signup.rateLimit` | json | `{"identityChecksPerHour": 10, "familyGroupLookupsPerHour": 20, "uploadUrlsPerHour": 30, "signupPerHour": 5, "signupPerDay": 20, "checkoutSessionsAnonymousPerHour": 10, "townsPerHour": 60, "notificationsPerRecipientPerHour": 3}` | sistema | S04 (`notificationsPerRecipientPerHour`: N-39 i l'N-01 de l'aplicant per destinatari, afegit 24-09 a proposta d'E3-T09) |

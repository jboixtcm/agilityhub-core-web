...
| 13 `mobil/13-els-meus-gossos.html` | `apps/clubs` | `/gossos` | MEMBER (i impersonat) | Una targeta per gos **propi** actiu (i una de reduïda per gos propi pendent de validació: nom, raça, sexe, edat i el xip «pendent de validació», sense accions; organitzador 24-09, E36): foto (toc → canviar-la), «{raça} · {sexe} · {n} anys», «Nivell {codi}» (si `levels.enabled`: `GET /me/dogs` només porta `level` llavors), «NOTES ALS INSTRUCTORS (què vols obtenir del club, aspectes a tenir en compte…)» (`TASKS`: text editable in situ, desa en perdre el focus o amb [DESA]), «TASQUES (marca-les quan estiguin fetes)» (`TASKS`: pendents amb casella, fetes ratllades «feta el {data}», clip d'adjunts, «Veure l'historial ›» → S10), documents «{nom} · {nom}» + «＋ DOC.» (diàleg: tipus de `documentTypes` de `GET /me/dogs` + nom + fitxer), «Pot entrenar sol» (verd, només si és cert), llicències «{organisme} · llicència {número} · {grau}», pack «{modalitat} · caduca {data} · {n} consumides · {n} disponibles» (`PACKS`, S12). «＋ AFEGEIX UN GOS» → S04. Peu fix: «El nivell l'assigna el club · Per donar de baixa un dels gossos, comunica-ho al club». Buit (cap gos actiu): text «Encara no tens cap gos donat d'alta» + «＋ AFEGEIX UN GOS». |
...
| `handlerName` | string ≤ 80 | no | **nom del guia quan no és l'abonat** (Jordi 09-09; columna «Nom del guia» de Playoff). Purament informatiu: no crea compte, ni permisos, ni reserva. R-03-19b |
...
| `licenses[]` | `[{organisation ≤ 20, number ≤ 30, category? ≤ 10, grade? ≤ 30, division? ≤ 20}]` | no | un registre per organisme (R-03-19); `category` (mida: `XS·S·M·I·L`) i `division` (`Iniciació·1D·2D·DH`) venen de Playoff (09-09) |
...
| R-03-15 | **Documents del gos**: tipus del catàleg `census.dogDocumentTypes` (`400 DOCUMENT_TYPE_UNKNOWN`); els tipus `required` es creen `PENDING` en registrar el gos (S04) si no s'han pujat. Fitxer: `mimeType ∈ files.allowedTypes` (`400 FILE_TYPE_NOT_ALLOWED`), mida ≤ `files.maxSizeMb` (`400 FILE_TOO_LARGE`), `name` demanat en pujar (per defecte el nom original), pujada per URL signada (`POST /attachments/upload-url {purpose: DOG_DOCUMENT}` → `fileKey`). Puja el `MEMBER` per als gossos propis (13 «＋ DOC.», amb els tipus de `GET /me/dogs` `documentTypes`: el MEMBER no llegeix `/parameters`) o l'ADMIN. Retirar un fitxer: només ADMIN, marca `removedAt` (l'objecte queda fins a la supressió RGPD, S14). `DogDocumentUploaded` en pujar; `DogDocumentPending` si un document requerit torna a PENDING. Un gos amb algun document requerit PENDING mostra «{tipus} pendent» a D15/D10/fitxa. | `census.dogDocumentTypes` (§13); `files.*`; `signup.requireDogDocumentAtSignup` = false | — | Duna: «cartilla_Duna_1.jpg · cartilla_Duna_2.jpg · Assegurança.pdf». |
...
| R-03-19 | **Llicències per organisme**: `licenses[]` amb `organisation` únic per gos (`400 VALIDATION_ERROR`), `number` obligatori, `category`, `grade` i `division` opcionals; tot text lliure amb suggeriments de `GET /dogs/filter-values?field=licenseOrganisation`. Les edita l'ADMIN (`PATCH /dogs/{id}`); l'alumne les veu («{org} · llicència {número} · {categoria} · {grau} · {divisió}», amb separador només dels camps informats; D15 «{org} {número} ({grau})»). | — | — | Rock: «FCAG · llicència 3241 · Iniciació» / «RSCE · llicència 13298 · M · 2 · 2D». |
| R-03-19b | **Guia del gos** (`handlerName`, Jordi 09-09). Quan qui condueix el gos a classe no és l'abonat propietari, l'ADMIN pot escriure'n el nom a la fitxa del gos (`PATCH /dogs/{id}`, auditat). **Convenció de visualització**: on el producte ja escriu «{guia} + {gos}» (21, 22, D12, D13, D15, cercador d'alumnes) el guia és `handlerName ?? Member.firstName`; quan difereixen, el backoffice hi afegeix «(abonat: {nom})» perquè no es perdi de qui és el gos. A l'app de l'alumne no canvia res. **No** dona accés ni permisos: la reserva i la responsabilitat continuen sent de l'abonat. A R1 no és al formulari d'alta pública (assumpció §13: el club l'omple després). | — | — | Toby, propietat de la Marta, el condueix la seva filla → «Toby · guia Júlia Roca» a la llista de classe. |
...
| R-03-23 | **Columnes i vistes**: cada llistat declara el catàleg de columnes (§6) amb visibles per defecte; l'usuari en tria i les reordena arrossegant; `SavedView` desa columnes, filtres i ordre; `name` únic per propietari i llista (`409 SAVED_VIEW_NAME_TAKEN`); `shared=true` la fa visible a tots els ADMIN/INSTRUCTOR del club, editable pel propietari o un ADMIN; claus de columna desconegudes s'ignoren en carregar. | — | INSTRUCTOR: crea/edita només les vistes pròpies i llegeix les compartides (precisió 09-09, E2-T07) | Vista compartida «Baixes previstes» = `displayStatus:eq:LEAVE_SCHEDULED`, ordre `leaveDate`. |
...
| GET | `/dogs` | ADMIN, INSTRUCTOR | — | sí | Llistat universal D15 | `x-filterable`: `name(contains), breed(contains), levelId, memberId, ownerName(contains), handlerName(contains), status, freeTrainingAllowed, hasLicense, licenseOrganisation, hasPendingDocuments, sex, birthDate, chip, registeredAt, levelAssignedAt`. `x-sortable`: `name, breed, levelOrder, ownerLastName, registeredAt, levelAssignedAt`. Columnes: `name*` Gos · `breed*` Raça · `level*` Nivell (`levels.enabled`) · `owner*` Abonat · `handler` Guia (només si difereix de l'abonat) · `freeTraining*` Entrenament lliure (`FREE_TRAINING`) · `licenses*` Llicències · `displayStatus*` Estat · `sex` · `age` · `chip` · `pendingDocuments` · `levelAssignedAt` Al nivell des de · `pack` (`PACKS`) · `registeredAt` | `200 {items: DogListItem[], …}` · `400 INVALID_FILTER` |
...
| PATCH | `/dogs/{id}/level` | ADMIN | — | no | R-03-12 | `{levelId}` | `200 {level, levelAssignedAt}` · `422 LEVELS_DISABLED \| LEVEL_NOT_ACTIVE \| LEVEL_UNCHANGED` |
...
| GET | `/me/dogs` | MEMBER | — | sí | Agregat de 13 (R-03-02, R-03-15, R-03-18) | — | `200 {dogs: [{id, status, name, breed, sex, ageYears, photoUrl, level?, instructorNote?, tasks?, documents[], freeTrainingAllowed, licenses[], pack?}], canAddDog, documentTypes: [{key, label, required}]}` — `level` només amb `levels.enabled`; `documentTypes` = el catàleg `census.dogDocumentTypes` amb `label` al `locale` del lector (R-03-32), per al diàleg «＋ DOC.» |
...
| T-03-19 | integració | When `PATCH /dogs/{id}/level {D}` sobre Duna (C, override null, D dona dret) · Then `levelHistory` amb 2 files, `DogLevelChanged` → N-09, `DogFreeTrainingChanged{true, LEVEL}`, cap avís ni `warnings` encara que tingui una classe de nivell C futura (la reserva es manté); mateix nivell → `422 LEVEL_UNCHANGED`; `levels.enabled=false` → `422 LEVELS_DISABLED` | R-03-12, R-03-13 |
...
| T-03-26 | integració | Given Laura (Duna, Rock; grup amb Joan Antoni/Toby) · When `GET /me/dogs` · Then 2 gossos, sense `chip`, `tasks` amb pendents + fetes ≤ 30 dies, `pack` només a Rock, `freeTrainingAllowed` coherent amb T-03-02, `licenses` de Rock, `documentTypes` amb les etiquetes en l'idioma de la Laura; Toby només a `/me/family-group` | R-03-02, R-03-15, R-03-18, R-03-19, R-03-11 |
...
- 08-09-2026 · respostes del Josep (B10): la fila «Mode de facturació» de D10 passa a **lectura**, derivada de `Plan.billingMode` (S05) — `Member.billingMode` es retira (l'alta en la modalitat Teràpia ja implica la quota de manteniment).
- 08-09-2026 · respostes del Josep (B19): R-03-12 — canvi de nivell amb reserves futures fora de nivell: **es mantenen i no s'avisa** ningú (es retira `warnings.futureBookingsOutsideLevel` de la resposta).
- 09-09-2026 · Jordi: **`Dog.handlerName`** (nom del guia quan no és l'abonat, R-03-19b + columna «Guia» a D15) i `licenses[]` amb `category` i `division` (columnes «Categoria RSCE» i «Divisió» de Playoff).
- 24-09-2026 · revisió de la porta E3: la pantalla 13 i `GET /me/dogs` inclouen els gossos propis pendents de validació, amb `status` i sense accions (E36).
- 25-09-2026 · organitzador (arran de l'informe d'E4-W08; tasques E3-T16 i E3-W12): el MEMBER, i la impersonació, no llegeixen `/parameters` (MATRIU_PERMISOS). `GET /me/dogs` retorna `documentTypes: [{key, label, required}]` per al diàleg «＋ DOC.» de 13, i la pantalla mostra «Nivell {codi}» quan el gos porta `level` (l'API l'omet amb `levels.enabled=false`).

...
| `submissionId` | string | l'enviament (`POST /signup` o `/me/dogs/signup`) que ha creat les línies de pagament; el d'un gos afegit va al `signup` del gos (§5; organitzador 24-09) |
...
| `UpfrontPayment` | `memberId`, `dogId`, `submissionId` (l'enviament que la crea; organitzador 24-09), `concept` (`ENTRY_FEE` · `FIRST_MONTH` · `PACK`), `amountDue`, `amountPaid`, `provider` (`STRIPE{checkoutSessionId, paymentIntentId}` · `MANUAL{channel?, paidAt}` · `null`), `status` (§5), `createdAt` | un registre per línia del bloc «Pagament inicial»; moviments immutables: les correccions són registres nous (S12) |
...
| **R-04-05 Persona existent** | `POST /signup/identity-checks` (i de nou dins `POST /signup`) cerca al club, entre `Member` `PENDING`/`ACTIVE`: `idDocument.value` igual **o** `contactEmails[0].email` ∈ emails de l'aplicant (només l'email principal: els segons emails poden ser compartits en família). Resultats: cap → `NEW` · `PENDING` → `SIGNUP_ALREADY_PENDING` (missatge «Ja tenim una sol·licitud pendent amb aquestes dades; el club la revisarà aviat», assumpció) · `ACTIVE` amb `Account` → `VERIFICATION_SENT`: s'emet `SignupRecognitionRequested{memberId, accountId}` (proposta §13) → enllaç màgic d'S01 (`auth.magicLinkMinutes = 15`, `redirect=/gossos/nou`) al correu principal de l'abonat; la resposta només porta `maskedEmail` («m•••a@e•••.cat») · `ACTIVE` sense `Account` → `CONTACT_CLUB`. El document mana sobre l'email. A `POST /signup` la mateixa coincidència → `422 SIGNUP_ALREADY_PENDING` / `409 MEMBER_ALREADY_EXISTS`. **Exemples:** Marta (activa) torna a omplir el pas 1 amb el seu DNI → `VERIFICATION_SENT`; Pau posa `pau@exemple.cat` (2n email de Marta) i el seu DNI → `NEW`; Pau posa `marta@exemple.cat` com a email principal → `VERIFICATION_SENT` a Marta (Pau necessita un email propi). |
| **R-04-06 Readmissió** | `POST /signup` amb l'`idDocument` d'un `Member` `LEFT` del club: no es crea cap registre; el mateix `Member` torna a `PENDING` amb `signup.readmission=true` i `number` conservat; els gossos antics segueixen `LEFT` llevat de R-04-07. **Les dades enviades (contacte, adreça, mètode de pagament, entrades noves de consentiments) no sobreescriuen la fitxa en enviar** (organitzador 24-09, E38): esperen a la sol·licitud; D2 mostra el distintiu «Readmissió» (literal nou, §13) i, per a cada camp canviat, el valor antic i el nou; la validació les aplica (auditat amb el diff emmascarat, S14 R-14-09). **El gos reutilitzat** (el xip d'un gos `INACTIVE` del mateix abonat, R-04-07) tampoc no se sobreescriu (organitzador 25-09, re-execució de la porta E3): passa a `PENDING`, però les dades enviades (nom, sexe, raça, mes de naixement, nota als instructors) i els documents esperen a la sol·licitud; els documents que ja tenia conserven els fitxers i l'estat; D2 en mostra el valor antic i el nou; la validació els aplica: cada tipus enviat amb fitxers substitueix el document del gos d'aquest tipus, i un tipus no enviat (o enviat sense fitxers) conserva el del gos (la fila buida automàtica de la cartilla només s'afegeix si el gos no en té cap). Mentre la readmissió està pendent, la fitxa del gos no es toca: D2 edita els valors i els documents enviats amb `PATCH /dogs/{id}` (per tipus), i les rutes que escriurien la fitxa responen 409 `INVALID_STATE` (`READMISSION_PENDING`): a D2, els documents, el recordatori de documents, la foto, el nivell, l'entrenament lliure, el traspàs, `handlerName` i `licenses`; les rutes pròpies de l'abonat (la foto, els documents, la nota als instructors i els seus adjunts); i les observacions (S10), quan s'implementin. Mentre esperen, els fitxers enviats compten com a referenciats per a P9 (S15 R-15-19). Un rebuig deixa la fitxa com era: torna a `LEFT` amb el `leftReason` i el `leftAt` originals, i el gos reutilitzat torna a `INACTIVE` amb el seu motiu, la seva data, les seves dades i els seus documents originals (els fitxers enviats queden orfes i P9 els esborra). L'enviament d'una readmissió s'audita. **Exemple:** Joan (núm. 214, baixa 2025) s'apunta amb el mateix DNI i un telèfon nou → Member 214 `PENDING`, amb el telèfon antic a la fitxa i el nou a la sol·licitud; en validar, conserva el 214 i passa a tenir el telèfon nou; si es rebutja, torna a la baixa del 2025 amb el telèfon antic. **Durant una readmissió pendent** (organitzador 24-09, revisió d'E3-T09): (a) la detecció de socis existents (R-04-05) també té en compte les adreces enviades; (b) el document d'identitat, i el xip del gos reutilitzat, no es poden editar des de D2 (409 `INVALID_STATE`, `details.reason = READMISSION_PENDING`): un DNI o un xip equivocat es resol rebutjant la readmissió; (c) N-01 i N-03 van a l'aplicant (§8): l'adreça principal enviada, en `signup.locale` i amb el nom enviat, sense cap dada de la fitxa de baixa; el destinatari viatja a l'esdeveniment, perquè un rebuig esborra les dades enviades; (d) la validació conserva el compte (`accountId`) de l'abonat si en té: no se'n crea cap altre, i una alta no canvia mai el correu d'accés d'un compte (és global, R-04-22). |
...
| **R-04-09 Modalitats mostrades** | `GET /signup.plans` = `Plan.active ∧ showOnSignup` ∧ tipus permès (`PACK` requereix `PACKS`; `SINGLE_CLASS` requereix `SINGLE_CLASS`; `MONTHLY` sempre), ordre del catàleg, amb la tarifa `STANDARD` vigent i l'entrada resolta = `Plan.entryFee.amount` ?? `billing.entryFeePerDog × percentOfStandard/100` ?? (`MONTHLY`: `billing.entryFeePerDog = 100 €` · `PACK`/`SINGLE_CLASS`: 0). Sense `BILLING`: mateixa llista sense imports. Llista buida → el bloc no es mostra i `planIdRequested = null`. Pla fora de la llista → `422 PLAN_NOT_AVAILABLE`. `priceLabel` = `Plan.texts.priceLabel` a l'idioma de qui llegeix (R-05-19). **Mode afegir gos** (organitzador 26-09): la llista inclou el pla de l'abonat, marcat `current: true`, encara que l'oferta pública l'amagui (M8); un abonat sense pla (B34), o amb un pla que ja no es pot assignar (inactiu o amb el mòdul desactivat), rep l'oferta, sense cap `current`, i `planIdRequested` és obligatori (`400 VALIDATION_ERROR`; organitzador 26-09, revisió d'E5-T22). **Cànic:** Abonat `MONTHLY` 60 €/mes + entrada 100 € · Pack 6 `PACK` 135 € (3 mesos) · Pack 10 `PACK` 180 € (5 mesos) · Teràpia `MONTHLY` amb `entryFee.percentOfStandard = 50` → 50 €, `maintenanceFee` 10 €/mes i sense tarifa estàndard («condicions i cost segons cada cas»). |
...
| **R-04-23 Rebuig** | `reason` 3–500 caràcters, obligatori. `PENDING` → `LEFT` (`leftAt`, `leftReason = SIGNUP_REJECTED`; una readmissió torna al `leftAt` i al `leftReason` originals, E38); tots els gossos `PENDING` → `INACTIVE{SIGNUP_REJECTED}`, llevat del gos reutilitzat d'una readmissió, que torna al seu `INACTIVE` original amb les seves dades i els seus documents (E38, R-04-06); `UpfrontPayment` `DUE`/`PARTIAL`/`CHECKOUT_PENDING` → `CANCELLED`, i una `PARTIAL` deixa una línia `PAID` de correcció pel que s'havia cobrat (E39b, §5) (els `PAID` es mantenen: D2 avisa «Hi ha un pagament cobrat: caldrà retornar-lo des de Facturació», reemborsament a S12); reclamació descartada; cap `Account`. Abonat `ACTIVE` (gos afegit): només els gossos `PENDING` → `LEFT` i els seus pagaments. `SignupRejected{memberId, dogIds, reason}` → N-03. Un `LEFT` rebutjat pot tornar a sol·licitar l'alta (R-04-06). Auditat amb el motiu. |
| **R-04-24 Antiguitat i recordatori** | Res no caduca ni s'esborra. `signup.submittedAt` alimenta el badge «pendent des de fa n dies» (avís si n > `dashboard.pendingSignupAgeWarnDays` = 2, com el «de fa més de {n} dies» de D1) i el procés diari `SignupPendingAging` → N-34 quan n > `signup.pendingExpiryDays = 30` (S15: `count`, `oldest_days`). Els fitxers d'una sol·licitud `PENDING` mai es netegen. |
| **R-04-25 Afegir un gos des de l'app** | `POST /me/dogs/signup` (MEMBER; impersonat → auditat amb els dos ids, `origin = BACKOFFICE`) exigeix `Member.status = ACTIVE`; crea `Dog` `PENDING`, `DogDocument`, `UpfrontPayment` `ENTRY_FEE` (si > 0) i emet `SignupSubmitted{memberId, dogIds, source: APP_ADD_DOG}` → N-01 (abonat: APP+EMAIL; admins). D1 el llista com «{nom} + {gos} (nou gos)». Validació amb el mateix `POST /members/{id}/validation` (abonat `ACTIVE`): `dogs[{dogId, levelId}]`, canvi opcional de `planId/priceId` (tarifa familiar), `nextInvoiceDate` opcional (es conserva), `upfrontAmountPaid` → gossos `ACTIVE`, `DogRegistered` per gos → N-37; **no** s'emet `MemberValidated` ni N-02 ni canvia número/compte. La pantalla 13 mostra el gos amb el xip «pendent de validació» i sense accions: `GET /me/dogs` retorna els gossos propis `PENDING` amb `status` (organitzador 24-09, E36). |
...
| **R-04-27 Tenant, idempotència, transaccions** | Host → club (`404 UNKNOWN_HOST` si no resol, S02); totes les consultes per `clubId` (`TenantRepository`); D2 sobre un abonat d'un altre club → `404 NOT_FOUND`. `POST /signup` i `POST /me/dogs/signup` accepten `Idempotency-Key` (24 h: la segona crida retorna el mateix `201`). Enviament, validació i rebuig en transacció Mongo amb l'outbox; validació i rebuig s'exclouen per `status` + `version` (`409 INVALID_STATE` / `409 STALE_VERSION`). `signup.enabled = false` o club no `ACTIVE` → `422 SIGNUP_CLOSED` (`GET /signup` ho anticipa amb `enabled = false` + `signup.text.closed`). |
...
Els imports no es modifiquen mai sobre un registre existent: un canvi de pla a D2 (`dryRun` → validació) **cancel·la** les línies `DUE` i en crea de noves; les `PAID` es conserven i es descompten del nou deure. Una línia `PARTIAL` que es tanca (canvi de pla o rebuig) passa a `CANCELLED` amb els imports intactes, i una línia nova `PAID` registra el que realment s'ha cobrat (`amountDue = amountPaid`, `correctionOf` = la línia tancada); aquesta compta com qualsevol `PAID`: es descompta del nou deure, o avisa del reemborsament en un rebuig. Si el cobrat supera el nou pressupost, no es crea cap línia i el `dryRun`/la validació avisen `PAID_EXCEEDS_QUOTE {amount}` (reemborsament a S12) — organitzador 24-09, E39b. Amb alguna línia `CHECKOUT_PENDING`, el canvi de pla es refusa (`409 INVALID_STATE`, `details.reason = CHECKOUT_PENDING`) i el `dryRun` ho avisa (organitzador 24-09, E39). Cada línia porta el `submissionId` de l'enviament que la crea: les d'una sol·licitud anterior (readmissió) no compten mai.
...
|---|---|---|---|---|---|---|---|
| GET | `/branding` | ANON | — | sí | S02: `modules[]`, `countryProfile`, `timeZone`, `locales`, tema | — | 200 · 404 `UNKNOWN_HOST` |
| GET | `/signup` | ANON · MEMBER | — | sí | configuració del formulari (una crida per pantalla) | → `{enabled, closedText?, steps[], plans[{id, type, name, description, conditions, offerLabel, price?, priceLabel?, entryFee?, pack?, maintenanceFee?, current?}], paymentMethods[{type, label, mandateText?, instructions?}], texts{freeTrainingConditions, therapyIntro, familyGroupIntro, monthlyPaymentIntro, paymentDay, cashConditions, imageConsent}, legal{privacyPolicyUrl, legalTextsVersion}, upfront{firstMonthSplitDay, today, firstMonthOptions[]}, member?{planId, paymentMethodMasked, consentsUpToDate}}` | 200 · 404 |
...
| POST | `/signup` | ANON | — | `Idempotency-Key` | enviament (cos a sota) | → `201 {memberId, signupToken, upfront{lines, totalDue}, checkout{required}}` | 400 `VALIDATION_ERROR` · 409 `MEMBER_ALREADY_EXISTS` · 422 `SIGNUP_CLOSED`/`SIGNUP_ALREADY_PENDING`/`PLAN_NOT_AVAILABLE`/`PAYMENT_METHOD_NOT_AVAILABLE`/`DOG_CHIP_ALREADY_REGISTERED`/`DOG_DOCUMENT_REQUIRED`/`CONSENT_VERSION_OUTDATED` (estats segons CATALEG_ERRORS regla 0, 09-09) · 429 |
...
| POST | `/me/dogs/signup` | MEMBER (impersonat: auditat) | — | `Idempotency-Key` | R-04-25 | `{dog, documents[], planIdRequested?, consents?}` → `201 {dogId, upfront, checkout}` | 400 · 422 `MEMBER_NOT_ACTIVE` · 422 (com `/signup`) |
...
| `billing.upfrontCutoffDay` (25), `billing.familyDiscountPercentFromSecondDog` (50) | R-04-14 mode «afegir gos» (Josep 08-09) |
...
| `signup.pendingExpiryDays` (30), `dashboard.pendingSignupAgeWarnDays` (2) | R-04-24 |
...
- T-04-06 (R-04-14) Línies: Abonat → `ENTRY_FEE` 100 + `FIRST_MONTH`; Pack 6 → `PACK` 135 sense entrada; Teràpia → `ENTRY_FEE` 50 sol; `BILLING` off → cap; mode afegir gos el 12-08 → `ENTRY_FEE` 100 + `ADDITIONAL_DOG_FEE` 30 (mes en curs) i, amb l'opció «l'1 de setembre», `ENTRY_FEE` 100 sol (el dia 26 aquesta opció ja no s'ofereix); pla amb `entryFee.amount = 0` → sense línia d'entrada.
...
- T-04-12 (R-04-06, R-04-07) Joan `LEFT` núm. 214 amb gos Bruc `INACTIVE` (xip X): `POST /signup` amb el seu DNI i xip X → cap document nou, Member 214 `PENDING` `readmission = true`, Bruc `PENDING` amb el nom i els documents antics a la fitxa i els enviats a la sol·licitud (D2 mostra l'antic i el nou); xip d'un gos d'un altre abonat → `422 DOG_CHIP_ALREADY_REGISTERED`.
...
- T-04-14 (R-04-10) `SEPA_DD` sense IBAN → 201 i `warnings` conté `ACCOUNT_NOT_PROVIDED` a `GET /members/{id}/signup` i al llistat de D1; IBAN amb mod-97 incorrecte → 400; `CARD` amb Stripe no actiu (`enabled: false`) → `422 PAYMENT_METHOD_NOT_AVAILABLE`; `GET /signup.paymentMethods[SEPA_DD].mandateText` cita la llei 16/2009 amb perfil `ES` i no amb `GENERIC`.
...
- T-04-19 (R-04-23) Rebuig: abonat `LEFT` i gossos `INACTIVE{SIGNUP_REJECTED}`, línies `CANCELLED` (una `PAID` es conserva i `warnings` ho diu), `SignupRejected` + N-03 a la cua; segon rebuig → `409 INVALID_STATE`; nova sol·licitud amb el mateix DNI → readmissió. Una readmissió que canviava el nom de Bruc, la raça i la cartilla, rebutjada → Bruc `INACTIVE` amb el motiu i la data de la baixa original, el nom i la raça antics i la cartilla antiga (fitxers inclosos, encara referenciats).
...
- T-04-21 (R-04-25) `POST /me/dogs/signup`: `Dog` `PENDING`, `SignupSubmitted{source: APP_ADD_DOG}`; validació → `DogRegistered` i **cap** `MemberValidated`, número i compte intactes; rebuig → només el gos `INACTIVE`; abonat `LEFT` → `422 MEMBER_NOT_ACTIVE`; token d'impersonació → `AuditEntry` amb `actorAccountId` + `impersonatedMemberId` i `origin = BACKOFFICE`.
...
- T-04-23 (R-04-27) Dues crides `POST /signup` amb la mateixa `Idempotency-Key` → mateix `memberId` i un sol document; claus diferents amb el mateix DNI → `422 SIGNUP_ALREADY_PENDING`; `signup.enabled = false` → `422 SIGNUP_CLOSED`.
...
**Tenant i rols**
- T-04-25 Host desconegut → `404 UNKNOWN_HOST`; sol·licitud del club A invisible des del club B (`GET /members/{id}/signup`, `validation`, `rejection` → 404); `MEMBER`/`INSTRUCTOR` a D2 → 403; token d'impersonació a `/members/{id}/validation` → 403; `ANON` a `/me/dogs/signup` → 401; `identity-checks` d'un DNI d'un altre club → `NEW`.
...
- 08-09-2026 · respostes del Josep (B13): R-04-14 mode «afegir gos» = `ENTRY_FEE` + `ADDITIONAL_DOG_FEE` del mes en curs, o només `ENTRY_FEE` amb efecte l'1 del mes següent si avui ≤ `billing.upfrontCutoffDay = 25`.
- 09-09-2026 · estats HTTP alineats amb la regla 0 de CATALEG_ERRORS en verificar E3-T01: `SIGNUP_CLOSED`, `SIGNUP_ALREADY_PENDING`, `DOG_CHIP_ALREADY_REGISTERED` i `ID_DOCUMENT_AMBIGUOUS` són **422** (no 409/400); `MEMBER_ALREADY_EXISTS`/`MEMBERSHIP_EXISTS` 409. Els exemples de §4 amb «409 SIGNUP_CLOSED» o «400 ID_DOCUMENT_AMBIGUOUS» s'han de llegir amb aquests estats.
- 16-09-2026 · implementació verificada (E3-T02/T03/T05): el model mana on S04 divergia — `Price` porta un `concept` (no un `scope` FAMILY/STANDARD) i la tarifa familiar és un `Plan`/`Price` elegible (`Plan.dogsIncluded`) que proposa el catàleg; `consents[]` canònic i només-afegir; el gos donat de baixa és `INACTIVE`; a la línia del segon gos el concepte de pagament del model és `OTHER` amb `signupConcept = ADDITIONAL_DOG_FEE` al contracte; `MAINTENANCE` no genera línia `FIRST_MONTH`; els dies de calendari es retallen a l'últim dia dels mesos curts; les versions de consentiment són opaques. Contracte ampliat: `AddDogSignupRequest.additionalDogOption` (TODAY · ALTERNATIVE), `SignupConfig.upfront.additionalDogOptions[]`, `SignupUpfront.additionalDog {option, startDate, amountDue}`, `AddDogSignupResult.checkout {required, memberId}`.
- 24-09-2026 · verificació d'E3-W03: l'IBAN emmascarat de D2 i de 19 (mode «afegir gos») segueix el format únic de R-03-27 d'S03. L'API no exposa el país ni els dígits de control (S03 §2: només els 4 últims), i el «ES02 ···· 7719» del mockup de D2 és il·lustratiu.
- 24-09-2026 · revisió de la porta E3 (`05-desenvolupament/backlog/revisio-porta-E3/`): E36 (gossos pendents a 13, R-04-25), E38 (readmissió sense sobreescriure la fitxa: R-04-06 i R-04-23), E39 i §5 (canvi de pla amb `CHECKOUT_PENDING`, línies `PARTIAL`, `submissionId`); estats segons el catàleg (`SIGNUP_CLOSED`, `SIGNUP_ALREADY_PENDING` i `MEMBER_NOT_ACTIVE` 422; host desconegut `404 UNKNOWN_HOST`, S02); l'avís d'antiguitat és `pendingDays > dashboard.pendingSignupAgeWarnDays` (2), a D1 i a D2.
- 24-09-2026 · verificació d'E3-W05: D2 mostra el DNI/NIE sencer, com D10 (R-03-27 només l'emmascara a la pantalla 28).
- 24-09-2026 · verificació d'E3-T09: R-04-06 (E38) precisa la readmissió pendent: la detecció de socis amb les adreces enviades, el DNI bloquejat a D2 (`READMISSION_PENDING`), N-01/N-03 a l'aplicant i el compte conservat.
- 24-09-2026 · verificació d'E3-T08: E39b (línia `PARTIAL` tancada → `CANCELLED` + línia `PAID` de correcció; `PAID_EXCEEDS_QUOTE`), R-04-13 per a grups de més gossos que cap tarifa familiar, `Member.signup.submissionId` a §3.
- 25-09-2026 · verificació d'E3-T14: T-04-14 parla de proveïdor **actiu** (`enabled`), no de «configurat»; `configured` no decideix l'oferta (R-04-10).
- 25-09-2026 · re-execució de la porta E3 (revisió d'E3-T11): E38 també cobreix el gos reutilitzat d'una readmissió, amb els seus documents (R-04-06, R-04-23, T-04-12, T-04-19): les dades i els documents enviats esperen a la sol·licitud i un rebuig el deixa com era. Codi: E3-T17 (api); el valor antic i el nou del gos a D2, al web després d'E3-T17.
- 26-09-2026 · verificació d'E3-T17: R-04-06 precisa el gos reutilitzat: la validació només aplica els tipus de document enviats amb fitxers; mentre la readmissió està pendent la fitxa del gos no es toca (D2 edita la sol·licitud amb `PATCH /dogs/{id}`, per tipus; les altres rutes que l'escriurien responen 409 `READMISSION_PENDING`); (b) també bloqueja el xip del gos reutilitzat.
- 26-09-2026 · verificació d'E3-T17 (ronda 2): R-04-06 anomena totes les rutes que una readmissió pendent congela per al gos reutilitzat.
- 26-09-2026 · verificació d'E4-W12: `GET /signup.plans` porta `priceLabel` (R-05-19) i, en mode afegir gos, el pla de l'abonat amb `current: true` (encara que l'oferta pública l'amagui); un abonat sense pla n'ha de triar un (R-04-09; E5-T22).
- 26-09-2026 · verificació d'E5-T22 i informe d'E4-W13: R-04-19 diu d'on ve un fitxer nou de D2 (la pujada de l'ADMIN amb `purpose = DOG_DOCUMENT`); R-04-09 tracta com a abonat sense pla el que té un pla que ja no es pot assignar (E5-T24).

...
| **R-01-06 Sessió persistent** | Refresh token opac (32 bytes) per dispositiu, `expiresAt = now + auth.sessionDays`, **lliscant**: cada `grant_type=refresh_token` rota el token i estén `expiresAt`; l'access token dura 15 min. Emmagatzematge a la PWA (**Jordi 06-09, A1 — supera l'storage WebCrypto**): el refresh token viatja i es guarda **només en una cookie `HttpOnly` + `Secure` + `SameSite=Strict`** del host de l'app (`Path=/oauth2/token`, `Max-Age = auth.sessionDays`), mai accessible des de JS; l'access token només en memòria. Perquè la cookie sigui *first-party*, cada host d'app (`app.*`, `admin.*` del club, `id.*`) serveix per **proxy same-site** (Caddy, A11) les rutes `/api/*` → core i `/oauth2/*` + `/.well-known/*` → id: l'SPA crida URL relatives i no hi ha CORS per als navegadors. Mecànica al servidor: els clients públics de navegador (`clubs-app`, `clubs-admin`, `id-web`) estan registrats amb `tokenDelivery = COOKIE` — la resposta de `/oauth2/token` **omet** `refresh_token` al cos i el posa a la cookie; `grant_type=refresh_token` sense paràmetre `refresh_token` el llegeix de la cookie (mateix client, `Origin` del mateix host); cada rotació reemet la cookie; `/oauth2/revoke`, logout i família revocada l'esborren (`Max-Age=0`). Clients `learn`/natius: `tokenDelivery = BODY` (com fins ara). Sense ús durant 30 dies → `401 REFRESH_EXPIRED` → 01. Dispositiu nou → 01. Nombre màxim de sessions per compte: `auth.maxSessions` (proposta, 10; en superar-lo es revoca la més antiga). | `auth.sessionDays = 30` | Laura entra el 5-10 i usa l'app el 20-10 → sessió fins al 19-11. |
...
| **R-01-15 `GET /me`** | Agregat d'arrencada de l'app: `{account {id, email, name, locale, platformRoles, hasPassword, emailVerifiedAt?, onboardingPending}, membership {clubId, roles, activeProfile, profiles[], memberId, instructorId, defaultProfile, rememberProfile, gender?}, impersonation?: {actorName}, features: branding.modules}` (06-09: `hasPassword` decideix si `PUT /me/password` demana `current` i si 02 mostra el bloc de contrasenya; `emailVerifiedAt` → badge «Compte activat»; `onboardingPending` → §14; `gender` ve de `Member.gender` — `MALE`/`FEMALE`/`OTHER` — i alimenta l'ICU de 02/03b; absent sense abonat) — la PWA el crida després de cada login/refresh i en tornar a primer pla. | — | — |
| **R-01-16 Seguretat de transport i claus** | TLS només; JWT RS256 amb rotació de claus (2 vius, `kid`), **claus privades en un anell xifrat AES-256-GCM a Mongo (`signing_keys`) amb la clau mestra `OIDC_MASTER_KEY` per entorn** (06-09, E1-T05 + A1; substitueix el PEM a `.env`/volum; rotació amb `identity:rotate-keys`, la clau anterior es conserva per verificar); secrets de clients `CONFIDENTIAL` hash bcrypt; CORS: hosts de clubs registrats + `id.*`; cookies de `id.*` `Secure` + `HttpOnly`; `Strict-Transport-Security`; cap token a URL llevat del codi de handoff/enllaç màgic (un sol ús, curts); logs sense tokens ni contrasenyes. | — | — |
...
Hosts: `id.agilitydoghub.com` per als endpoints OAuth2/OIDC (`/.well-known/*`, `/oauth2/*`, `/connect/logout`) i `apps/id`; **la resta a `core.*/api/v1`** (també `/auth/magic-link` i `/auth/handoff`: són API d'aplicació, no OAuth2). Rols: `ANON` = sense token; `compte` = qualsevol token vàlid.
...
| POST | `/oauth2/revoke` | compte | sí | revoca refresh o impersonació | `{token}` — o **`{}` amb el bearer** per als clients `tokenDelivery = COOKIE` (la cookie no arriba a aquesta ruta pel seu `Path`): el servidor revoca la família de sessió del bearer i esborra la cookie (09-09, E1-T13) | `200` |
...
| POST | `/oauth2/session` | compte (`id-web`, mateix origen) | — | 06-09 (E1-T05): pont entre el login de l'SPA `apps/id` i la sessió de navegador OIDC — `/oauth2/authorize` sense sessió redirigeix a `id.*/login?flow=…` i deixa una cookie de flux (5 min); després d'entrar, l'SPA hi envia el seu bearer + `flow` i rep la URL on continuar | `{flow}` | `200 {redirectUrl}` · `400 VALIDATION_ERROR` (flux caducat/desconegut) · `401` · `403` |
...
| GET | `/me/onboarding` | compte | — | §14 (06-09): estat de l'onboarding i del consentiment vigent | — | `200 OnboardingState {pending, postponeRemaining, requiredConsent: {policy: PLATFORM·CLUB, version, url} \| null, fields[]: {key, value, required}}` (`fields` segons `signup.onboardingFields`, valors preomplerts d'`Account`/`Member`) |
| PUT | `/me/onboarding` | compte | — | §14: acceptació + dades opcionals | `{consentAccepted: true, consentVersion, fields?: {name?, locale?, phone?}, imageConsent?}` | `200 OnboardingState` · `400 VALIDATION_ERROR` (casella no acceptada) · `422 CONSENT_VERSION_OUTDATED` (versió diferent de la vigent) · `400 LOCALE_NOT_SUPPORTED` |
| POST | `/me/onboarding/postpone` | compte | — | A12: «Més tard» | — | `200 OnboardingState` (decrementa `postponeRemaining`; a 0 retorna l'estat sense canvis, cap error) |
| POST | `/members/{id}/impersonation-token` | ADMIN (no `imp`) | — | R-01-09 (definit a S03, implementat aquí) | `{reason?}` | `201 {token, expiresAt, launchUrl}` (06-09, alineat amb S03 §6: `launchUrl` = `https://{host de l'app de club}/entrar?handoff=<codi d'un sol ús, 60 s>` lligat a la sessió impersonada — mai el JWT a la URL) · `403 IMPERSONATION_DENIED` · `404` · `409 MEMBER_NOT_ACTIVE` (S03) |
...
|---|---|---|---|
| 1 | ~~Emmagatzematge del refresh a la PWA: storage xifrat o cookie via proxy `same-site`?~~ **Resolt (Jordi 06-09, A1)**: proxy same-site + cookie `HttpOnly` (R-01-06); el WebCrypto es retira amb E1-T13/E1-W07 | Jordi | — |

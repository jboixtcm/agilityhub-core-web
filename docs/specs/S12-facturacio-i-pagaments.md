...
| **R-12-05 Efectiu** | `MANUAL`: dues branques de producte, totes dues **editables des de D11** (bloc «Quotes, packs i remesa»; Jordi 09-09, A29). `billing.cashInvoicing = SEMESTER` (**Cànic, Josep 08-09: en efectiu no hi ha quota mensual**) → un rebut `PENDING` amb `k` línies mensuals (`k` = mesos que falten fins al final del **semestre natural**, `billing.cashPeriodMonths = 6`: gener–juny i juliol–desembre) i `nextInvoiceDate` → dia 1 del semestre següent; la **primera fracció** d'un abonat nou són els mesos que van del seu `startDate` fins al final del semestre en curs, i a partir d'aleshores semestres complets. L'admin marca el rebut cobrat. `MONTHLY` → un rebut d'una línia cada mes; és igualment de producte i està provat (T-12-03), no és «la branca dels altres clubs». El text de l'alta és el paràmetre `signup.text.cashConditions` (`localizedText` per club, S04) i **s'ha de repassar en canviar aquesta clau**; el del Cànic («semestres naturals complets») descriu exactament el cas `SEMESTER`. | `billing.cashInvoicing = SEMESTER`, `billing.cashPeriodMonths = 6` | Joan Vila (efectiu, alta el 17-08): rebut «Quota Abonat — Agost a Desembre 2026 · 5 línies · 300,00 € · Efectiu · pendent · marca cobrat»; el següent, l'1 de gener, de 6 mesos. |
...
- T-12-03 (R-12-05) `cashInvoicing = SEMESTER` (Cànic): alta el 17-08 → 5 línies (agost–desembre) i `nextInvoiceDate` 01-01; run del gener → 6 línies i `nextInvoiceDate` 01-07; alta a l'octubre → 3 línies. `MONTHLY` → 1 línia (branca de producte igualment suportada).
...
- T-12-29b (R-12-12) `billing.sepa.collectionDayOfMonth`: amb `1`, el run del 25-08 per a setembre dona `ReqdColltnDt = 2026-09-01`; amb `0` (últim dia), `2026-09-30`; amb `28` al febrer, `2026-02-28`; si la data resultant deixa menys de 2 dies hàbils des de la generació → `422 COLLECTION_DATE_TOO_SOON`. Canviar la clau des de D11 (`PUT /parameters/billing.sepa.collectionDayOfMonth`) no recalcula cap remesa ja generada (R-02-04).
...
|---|---|---|---|
| 1 | ~~Efectiu: rebut mensual o per semestres?~~ **Resolt**: al Cànic sempre per semestres naturals (Josep 08-09); **Jordi 09-09 tanca A29** — les dues branques són de producte i s'editen des de D11 | — | `billing.cashInvoicing = SEMESTER` al Cànic (`MONTHLY` suportat i provat); el text de la pantalla 19 es manté i es repassa (`signup.text.cashConditions`) en canviar la clau |
...
| 3 | ~~Data de cobrament de la remesa?~~ **Resolt (Josep 08-09): el dia 1 del mes que es factura** («la quota d'octubre es cobra l'1 d'octubre»; **Jordi 09-09 tanca A28** — les dues semàntiques són de producte i s'editen des de D11) | — | `billing.sepa.collectionDayOfMonth = 1` al Cànic, semàntica «dia del mes facturat»; `0` (últim dia) suportat i provat (T-12-29b); editable des de D11 i en generar |
...
- 09-09-2026 · **Jordi tanca A28/A29**: no són decisions d'operativa sinó **valors per club**. `billing.cashInvoicing` i `billing.sepa.collectionDayOfMonth` viuen al bloc «Quotes, packs i remesa» de D11 amb les dues branques implementades i provades (T-12-03, T-12-29b); el Cànic manté `SEMESTER` i `1`. En canviar-les cal repassar `signup.text.cashConditions` i `signup.text.paymentDay` (S04, pantalla 19). Criteri: «cada club és un món».

...
| **R-12-02 Línies del mes** | Segons `Plan.type` del membre: `MONTHLY` → una línia `MONTHLY_FEE` amb `PriceResolver.current(planId, MONTHLY_FEE, issueDate)` (S05), **substituïda** per `INACTIVITY_FEE` si `InactivityFeeService.feeFor(memberId, M)` retorna import (S13 R-13-08), o per `MAINTENANCE_FEE` si `Member.billingMode = MAINTENANCE` (teràpia: quota mínima, assumpció §13) · `PACK` → cap línia periòdica · `SINGLE_CLASS` → una línia per `PendingCharge` no facturat (R-12-25). Import 0 o cap línia → **no es genera rebut** i `nextInvoiceDate` avança igualment. Descripció congelada: «Quota {pla} — {Mes any}» / «Quota inactivitat — {Mes any}» / «Classe {data} — {gos}» en `defaultLocale` del club. | `BILLING`, `INACTIVITY`, `SINGLE_CLASS` | Eva (inactiva des de l'agost): setembre → «Quota inactivitat — Setembre 2026 · 10,00 €». |
...
| **R-12-05 Efectiu** | `MANUAL`: `billing.cashInvoicing = MONTHLY` (proposta §13, valor del mockup) → rebut mensual `PENDING` que l'admin marca cobrat; `SEMESTER` → al primer mes de cada semestre natural un rebut amb `k` línies mensuals (`k` = mesos fins a final de semestre) i `nextInvoiceDate` → dia 1 del semestre següent. El text de l'alta («semestres naturals complets») és informatiu. | `billing.cashInvoicing`, `billing.cashPeriodMonths = 6` | Joan Vila (efectiu): «Quota Abonat — Setembre 2026 · 60,00 € · Efectiu · pendent · marca cobrat». |
...
| POST | `/webhooks/stripe/{clubId}` | Stripe (signatura) | per `eventId` | R-12-21 | payload Stripe | `200` · `400 WEBHOOK_SIGNATURE_INVALID` · `404` (club sense Stripe) |
...
- T-12-03 (R-12-05) `cashInvoicing = MONTHLY` → 1 línia; `SEMESTER` al juliol → 6 línies i `nextInvoiceDate` 01-01; a l'octubre → 3 línies.
...
|---|---|---|---|
| 1 | ~~Efectiu: rebut mensual (mockup D6) o per semestres (text de l'alta)?~~ **Resolt (Jordi 05-09): mensual** | — | `billing.cashInvoicing = MONTHLY`; el text de semestres de l'alta es revisa amb el Josep (B22) |
...
| 3 | ~~Data de cobrament de la remesa: sempre dia 1 del mes següent?~~ **Resolt (Jordi 05-09): l'últim dia del mes** | — | `billing.sepa.collectionDayOfMonth = 0` (= últim dia del mes; 1–28 per a un dia fix), editable en generar |
...
| 6 | Teràpia: quan s'aplica la quota de manteniment (`Member.billingMode = MAINTENANCE`)? Qui ho canvia? | Josep | commutador a D10 (ADMIN), per defecte `MONTHLY_FEE` |

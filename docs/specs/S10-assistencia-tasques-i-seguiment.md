...
| Camp | Tipus | Oblig. | Validació / notes |
|---|---|---|---|
| dogId, memberId | UUID | sí | gos `ACTIVE` en crear (`422 DOG_NOT_ACTIVE`); `memberId` = propietari (es refresca amb `DogTransferred`: la tasca segueix el gos, S03 R-03-14) |
...
| PUT | `/class-sessions/{id}/attendance` | INSTRUCTOR, ADMIN | — | sí | [DESA] / [DESA LA LLISTA] (R-10-04) | `{version, items[{bookingId, state}]}` | 200 (com el GET + `applied[]`) · 409 `STALE_VERSION{current}` `INVALID_STATE` · 422 `ATTENDANCE_NOT_OPEN` `ATTENDANCE_WINDOW_CLOSED` `ATTENDANCE_NOTIFIED_FINAL` `ATTENDANCE_BOOKING_NOT_ACTIVE` `INSTRUCTOR_NOTICE_DISABLED` · 400 estat desconegut |
...
| POST | `/tasks` | INSTRUCTOR, ADMIN | `TASKS` | sí | «＋ Afegir» | `{dogId, text, attachmentIds?[]}` | 201 `Task` · 400 · 422 `DOG_NOT_ACTIVE` |
...
| POST | `/tasks/{id}/completion` | MEMBER (propi, impersonat), INSTRUCTOR, ADMIN | `TASKS` | per estat | marca feta | `{}` | 200 `Task` · 422 `TASK_ALREADY_DONE` · 404 |
| POST | `/tasks/{id}/reopening` | INSTRUCTOR, ADMIN | `TASKS` | per estat | torna a pendent (assumpció) | `{}` | 200 · 422 `TASK_NOT_DONE` |
...
```json
{ "dog":{"id":"d1","name":"Duna","handlerName":null,"breed":"Border collie","sex":"FEMALE","ageYears":4,"photoUrl":"https://…","status":"ACTIVE"},
...
- T-10-11 (R-10-04, R-10-05) `PUT` amb `NOTIFIED` a 16:00: reserva `CANCELLED`, `PackRefunded`, `SeatReleased{notifyWaitlist:true}`, `WaitlistNotified` + N-15 (una vegada), N-05 sense SMS, `AttendanceMarked{NOTIFIED}`, resposta amb `notice.seatReleased=true, waitlistNotified=true`; a 18:25 → `CANCELLED_LATE`, sense pack ni N-15; a 20:30 → `afterClassEnd`, reserva `ACTIVE`, cap esdeveniment de S08; `instructorLastMinuteNotice=false` → `422 INSTRUCTOR_NOTICE_DISABLED` i res canvia.
- T-10-12 (R-10-04) `PUT` amb un ítem invàlid entre tres → 422 i **cap** dels tres aplicat (rollback); `version` antiga → `409 STALE_VERSION{current}`; mateixa `Idempotency-Key` → mateixa resposta i un sol `AttendanceMarked` per ítem; `attendanceSummary` coherent després de cada desat.
...
- T-10-15 (R-10-10) `POST /tasks` → `TaskCreated` + N-20 encuada al propietari (no al grup); text buit → 400; gos `INACTIVE` → `422 DOG_NOT_ACTIVE`; `PATCH` amb `version` antiga → 409; `DELETE` → `deletedAt`, no surt a `GET /tasks` ni a D14, sí amb `includeDeleted` (ADMIN); `completion` per MEMBER propietari (i impersonat, auditat) → N-21 a tots els instructors actius amb `member_name`; per un gos del grup → 404; segon `completion` → `TASK_ALREADY_DONE`; `reopening` per MEMBER → 403.
...
- 24-09-2026 · verificació d'E6-T01: estats HTTP alineats amb la regla 0 de `CATALEG_ERRORS.md` (els codis de negoci d'S10 són **422**; `STALE_VERSION` i `INVALID_STATE` continuen 409). `handlerName` a l'exemple de la fitxa (R-10-00). Dependència amb S03 per `DogOwnerAccess` (§7).

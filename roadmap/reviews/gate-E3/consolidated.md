...
| **E3-T16** | api | The member's document types in `GET /me/dogs`: screen 13 cannot read `/parameters` (added 25-09, from the E4-W08 report); the Cànic's legal identity, `displayCity` and `legalAddress` for the public footer (Jordi, 25-09); the E3-T15 review's test gaps and the list `size` contract | E3-T10 | in progress |

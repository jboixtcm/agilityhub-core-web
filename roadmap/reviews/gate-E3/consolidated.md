...
| Task | Repo | Content | Depends on | Status 24-09 |
|---|---|---|---|---|
| **E3-T07** | api | Baseline audit run on `de0e17f` | — | verified |
| **E3-T08** | api | The smoke (step 1); M5 api side (a quote per plan in `GET /signup`); M6, M7, M8 (+ `planOptions`), M9, M10 (E39), M11 server side (+ `warnDays` in the signup view), M20 api side (E36), M21; the dog `version`; the signup flags; E35 | E3-T07 | ready |
| **E3-T09** | api | M16 api side (+ a MinIO IT), M17, M18 (E38); the security minors | E3-T08 | ready |
| **E3-T10** | api | The other api minors, the event payloads, the `Member.signup` snapshot, the OpenAPI error lists, the D1 details (`leftAt`, `waitingTotal`, `TASKS` gating, `percent`), and the weak tests | E3-T09 | ready |
| **E3-T11** | api | Audit re-run after the fixes | E3-T07…T10 | not_open (the organizer opens it) |
| **E3-W05** | web | Baseline audit run | — | running |
| **E3-W06** | web | B1, M1, M2, M19, the payment texts under the right method; the signup minors (no api change needed) | E3-W04 | ready |
| **E3-W07** | web | Snapshot adoption; D2: M12–M15, M11 web side, E38 old/new, the refund warning; the D2 and D1 minors | E3-W06, api E3-T08/T09 | not_open (opened when the api publishes) |
| **E3-W08** | web | M3, M4, M5 from the per-plan quote; M16 web side; M20 web side (E36); the signup flags | E3-W07, api E3-T08/T09 | not_open |
| **E3-W09** | web | Audit re-run after the fixes | E3-W05…W08, api E3-T10 | not_open |
...
**Closing the gate:**
- The organizer verifies the six fix tasks and the two audit re-runs.

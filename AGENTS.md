...
2. **No literal colors** outside `packages/ui/src/tokens.css`; everything themed through CSS variables (`--ah-*`) so each club's theme applies. Lint enforces it.
   **White-label, no club literal anywhere in code or locales**: the mockups are the Cànic's instance of a generic product. Club names, towns, taglines, wordmarks («CÀNIC AGILITY», «escola canina», «Club Agility …», «Cabrera de Mar») and slug conditionals (`slug === "canic"`) never appear in `apps/*`, `packages/*` or `locales/*`; they live only in the branding contract (`club.name`, `club.city`, `theme.logoUrl/logoDarkUrl/markUrl`) and in MSW fixtures/seeds. Brand text is an image (the club's logo), never typed text. Placement rule: compact spots (shell header/sidebar, < 48 px high) show `markUrl` + `club.name`; large spots (access screens, cards) show the full logo (`logoDarkUrl` in dark mode, else `logoUrl`); fallbacks mark + name, then initial + name — all through `resolveBrandingLogo` in `packages/ui`. When a mockup shows something club-specific, ask «which branding field carries it?» — if none, propose the field in the report instead of hard-coding.
...
4. **Types from OpenAPI** (`packages/api-client/src/generated`), never hand-written API types. Errors are `ApiError {code, message, details, traceId}`; map codes to i18n messages.
   Contract files: `packages/api-client/openapi/openapi.json` is the api's verified snapshot (copied by the organizer, **never edited by hand**); `pending.json` holds only mocks-first contracts the api has not published yet (the generator fails if it redefines something already in the snapshot); when the organizer stages a newer snapshot as `openapi.next.json`, only the dedicated adoption task swaps it in and prunes `pending.json`.
...
4. Run every **Verification** command; put the evidence in the task's **Executor report** and attach screenshots (`roadmap/evidence/<ID>/*.png`, committed). **Evidence rule**: for every Verification command paste the exact command, its exit code and the **last 40 lines** of its output; when the output is longer, write the complete output to `roadmap/evidence/<ID>/NN-<name>.log` (committed; tokens/secrets truncated; `.gitignore` un-ignores `roadmap/evidence/E4-*/*.log` … `E12-*`, never add an ignore rule for them) and reference the file. A task file must stay under **120 KB** (`check.py` refuses `awaiting_verification` above that): earlier failing attempts get one line each (what failed → what you changed) plus their log file; only the final run keeps its tail in the report.
5. **Before** `awaiting_verification`: run the complete Playwright suite (`pnpm e2e`, the three projects) in the Playwright Docker image, not only the spec you added — CI runs it all on `main`, and a task that breaks another app's e2e comes back as `changes_requested` (09-09: E2-W06 broke the E2-W01 export test). Then fill the report, update `CHANGELOG.md`, `check.py --set <ID> awaiting_verification`. The publish script commits and pushes after the session (CI runs on `main`).
